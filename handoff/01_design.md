...
> ⚠️ **v3 갱신 — 에디션(연도판) 층 도입.** 아래 §3.1–3.2 의 2층 모델은 **에디션 3층 모델로 대체**되었습니다.
> `lineage_lists`(시리즈) → `lineage_editions`(연도판; **연도는 항상 여기**) → `film_lineage`(소속/결과; `result`/`rank` 1급).
> `label_ko` 제거. 권위 있는 최신 스펙은 **`00_INTERFACE_CONTRACT.md` + `02_schema.sql`** 를 따르세요. (이 문서는 분석·근거 참고용.)
