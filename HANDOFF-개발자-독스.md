...
> 🛑 **상태: 보류 (ON HOLD) — 2026-07-12 오너 결정. 이 계획을 구현·저술하지 말 것.**
> **보류 사유(요지)**: 사이트 전체 권위는 "AI 초안이되 엄격히 통제·인간 검수된, 슬롭이 아님"이라는 로드베어링 기둥 위에 있다. "vibe coding" 프레임(부정확하고 권위-독약)을 초대하는 개발 뒷이야기 공개는, 아직 형성 중인 콘텐츠 권위를 오염시킬 하방이 개발 트래픽이라는 상방을 초과할 수 있다. → 지금은 하지 않는다.
> **되살릴 조건(강한 위치에서만)**: ① 영화 편집 권위가 더 단단해진 뒤, ② 규모 대폭 축소(46편→rigor-증명 4~6편만), ③ **결정 규칙**: "사이트가 콘텐츠에 대해 주장하는 rigor를 *증명하는* 글만 — 그런 글은 신뢰의 증거가 되고, 나머지(해키-옵스: bash CI/CD·schema-in-VCS-gap·자작 렌더러)는 신뢰를 깎으니 제외", ④ "AI로 지었다/vibe"를 헤드라인으로 내세우지 않기(방법론 독스가 이미 콘텐츠의 슬롭 여부를 인간-검수 방화벽으로 처리함), ⑤ 편집자 Wonwoo와 빌더 Wonwoo 방화벽 분리(개인 엔지니어링 노트로 명확히 경계). 아래 계획은 보존용이며, 되살릴 땐 이 5조건으로 §7을 먼저 재큐레이션한다.
...
- 2026-07-12 — **오너 결정: 보류(HOLD).** 이유를 전략 분석까지 남긴다(미래 판단용):
  - **문제 제기(오너)**: 이 사이트가 "vibe coding의 결과"라는 것을 여기서 밝히는 것이 사이트 *전체 권위*에 해가 되고, 나중에 신뢰 문제로 이어질 수 있다.
  - **분석**: (1) 진짜 오염 경로는 "엉성한 엔지니어링→엉성한 비평"이 아니라, 콘텐츠 권위의 로드베어링 기둥인 **"슬롭 아님(엄격히 통제·인간 검수)"**을 "vibe coding" 프레임이 직접 때리는 것 — "코드를 vibe로 짰다면 비평도 vibe로 뽑았겠지". (2) 그러나 실제 프로세스(HANDOFF·불변식·검증·신뢰도 측정·빌드게이트·라이브검증)는 vibe coding이 **아니라** 규율 있는 AI-보조 엔지니어링이며, 작동하는 7,000페이지 사이트가 그 증거. 문제는 실체가 아니라 **라벨과 프레이밍**. (3) "vibe coding" 라벨은 부정확할 뿐 아니라 영구 앵커가 되는 권위-독약 → 채택·초대 금지. (4) 결정 규칙 도출: *사이트가 주장하는 rigor를 증명하는 글만 안전(신뢰의 증거) / 나머지(해키-옵스)는 사이트-신뢰 리스크*. (5) 타이밍: 권위가 형성 중이라 지금은 하방>상방. 영화 권위 확립 후 강한 위치에서 축소판으로.
  - **결론**: 46편·AI-빌더 전면 브랜딩 형태로는 **하지 않는다.** 되살린다면 상단 배너의 5조건으로 재큐레이션. 대안이었던 (A)대폭축소+리프레임 / (B)보류 중 오너가 **(B)** 선택.
