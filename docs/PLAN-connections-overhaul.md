...
| **3. Galaxy** | t-SNE(cosine, seed 42) 2D 좌표 `film_map_xy` 1,941 + KMeans 14클러스터 + 라벨(`film_map_clusters`: 장르쌍+대표트롭, RPC `galaxy_refresh_cluster_labels`). 빌더 `worker/galaxy-build.py`. API `/api/map/galaxy`(galaxy_json RPC, s-maxage 3600). UI `components/GalaxyMap.tsx`(캔버스, rAF 루프 없음—이벤트 드리븐) + /map 4번째 탭 "Galaxy"(`?m=galaxy` 딥링크). **7/5 원우 피드백 반영**: Atlas식 좌측 패널("N films in view" + Year↓↑/Title/Neighbourhood 정렬, 행 hover=점 링, 클릭=영화 페이지) + 화면에 ≤70점일 때 캔버스에 제목 라벨 직접 표기. | 클러스터 분포 87~182 균형, 라벨 유의미("Crime · Thriller — Hopper's Lonely American Light") |
...
- **후속 미세수정(같은 날)**: ① 캔버스 얼굴/포스터 drawImage를 cover-crop(9-인자)으로 — 세로 원본을 정사각에 그리던 가로 왜곡 해소(얼굴은 상단 18% 바이어스) ② movies-like Shares를 "{후보} and {원본} share: …"로 양편 명시 ③ 랭킹 숫자 aria-hidden 해제 + "#N" 실텍스트(SEO 가독).
