...
> ⚠️ **2026-07-12 갱신 — 실행 지침서 분리.** 오너 결정(Pass $9/월 채택·$1 단품 폐기 / L0 복사 무로그인 확정 / 풀팩 월 10 무료 쿼터 / 마이룸 무료 유지+`/room/packs` 라이브러리 / 구글드라이브 보류·MCP 우선)과 DB/코드 실측 정정(**geo_cache 100% Google→v1 팩에 좌표 전면 금지**, 클릭 로깅은 content_events가 아니라 **mt_events** `mtEvent()`, 심층 코퍼스 1,938편·편당 평균 14 takes, 마이그레이션 다음 번호 **0085**, 풀팩 RPC는 anon GRANT 금지=DB 엣지가 유료 경계, `/api/films/backfill`은 클라이언트 호출이라 시크릿 게이트 불가)을 반영한 **실행 정본은 루트 `HANDOFF-컨텍스트팩-실행.md`**다. §8 W1의 세부와 충돌하면 실행 지침서가 우선한다. 실행 세션은 그 문서에서 시작할 것.

> ⚠️ **2026-07-13 갱신 — "MCP 우선"이 실행됨.** 위 오너 결정의 "구글드라이브 보류·MCP 우선"이 구현·배포·등록까지 완료: 공개 **MCP 서버 `/api/mcp`**(팩 렌더러 재사용) + 공식 MCP Registry `net.metatake/mcp` + 3층 수확방어(0091) + 전량 원장(0093 `mcp_calls` = API 승격 판정의 수요 신호) + AI 유입 측정(0092). **정본은 루트 `HANDOFF-MCP-서버.md`.** 팩 라우트도 동작 변경(fmt=json 로그인 게이트) — 컨텍스트팩 실행 지침서 상단 델타 배너 참조.
