...
> **Big shift 2026-07-17 — 앱 디자인 대상급 재설계 + Android 패리티** (정본 `HANDOFF-모바일앱-프리워치.md` v3.0 §3·§8): 오너 지시로 **디자인 시스템 v2 "Lava"**(벤치마크=Airbnb 2025 Lava + iOS 26 Liquid Glass) 전 화면 적용 — 웜 중립 팔레트 + **그라데이션 CTA 하나**(#FF385C→#D70466)·동심 라운드(8/14/24/999)·소프트 섀도·**블러 반투명 탭바**·스프링 프레스·문장 케이스 워딩·시트 오버 포토 Film 카드·벤치마크 로그인 순서(이메일→그라데이션 Continue→or→Apple 먼저)·**브랜드 아이콘 세트**(TakeScore 링+세리프 M on Lava). ⚠️**웹 DESIGN-SYSTEM v4 이식판(직각·헤어라인·#E3120B)은 앱에서 폐기** — 웹=에디토리얼 아카이브, 앱=결정 도구. PT Serif는 작품 제목·Invitation에만 생존(웹 정본은 불변). 동시에 **Android를 iOS와 같은 커밋에서 패리티 완성**: Expo 단일 코드베이스(화면·로직 100% 공용) + 분기 4개(지도·블러·로그인·딥링크). **Android Expo Go 지도는 WebView MapLibre**로 Google Cloud 키 없이 실지도(react-native-maps=Android는 Google 지도라 키 필수 → 회피). edge-to-edge·키보드 pan·적응형 아이콘·eas.json 양 플랫폼(dev/preview APK, prod AAB). 검증: tsc 0 · **3플랫폼 번들 전부 빌드** · 브라우저 E2E 에러 0 · Android WebView 지도 실측(800핀 클러스터+핀 탭 브리지). 남은 오너 몫: Apple $99·Play $25·eas init·FCM/APNs·assetlinks SHA256.

> **Big shift 2026-07-16~17 — 모바일 앱 "Pre-Watch Companion" 시제품** (정본: 루트 `HANDOFF-모바일앱-프리워치.md` v2.3 — §15 AS-BUILT·§16 데이터 연동 지도; 브랜치 `claude/cinephile-mobile-app-dphwc2`, PR #7): 프리워치 결정 루프(볼까→뭘→어디서→나중에)를 담은 **Expo(SDK 57) iOS-우선 앱 P0~P3 코드 완성 + 완전 동작 시제품 검증**(브라우저 E2E·콘솔 에러 0). 사이트 쪽 신설: **모바일 BFF 7라우트** `/api/v1/app/{film,director,tonight,services,handoff,account-delete,tmdb-search}`(화면당 1응답, guardAndLog → api_calls에 `app_*` 엔드포인트로 계측), `/api/lens/marquee` Bearer 폴백(가산적), **푸시 크론** `/api/push/availability-cron`(vercel.json 매일 09:00Z — watchlist×fpi 국가별 diff), **마이그 0106**(`push_tokens`·`user_prefs`·`push_sent`, own-row RLS — **0105는 다국어 프로젝션이 선점, 갭 의도**), `.well-known` AASA/assetlinks(TEAMID 플레이스홀더). 데이터 원칙: 앱 전용 데이터 0 — 찜/Seen은 `user_movies` **단일 원장**(웹 렌즈와 즉시 동기, 앱은 행 삭제 안 함), 콘텐츠·랭킹·가용성 전부 기존 RPC 소비(cinecodex_ranked v11 인자면·takescore_for_slugs·film_availability·film_geo·film_lineage_for·search_all). 지도는 3면(웹 maplibre-gl JS / Expo Go Apple 지도 / dev빌드 MapLibre 네이티브, 공유 계약 `mobile/src/lib/pins.ts`). 다국가 에디션(US live·KR/ES/JP 예약)은 웹 LOCALES 레지스트리와 결합. ⚠️읽기 표면 라우트 개명 시 앱 웹뷰 경로 3파일 동기 필요(§16.5 — 유일한 수동 결합점). 남은 오너 몫: Apple Dev·eas init·AASA TEAMID·Supabase provider/OTP 템플릿·TestFlight KPI 게이트(§9).
