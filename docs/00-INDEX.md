...
- `docs/PLAN-intent-coverage.md` — **인텐트 커버리지 / Quick Answers 층 (설계+전유형 진단 완료 2026-07-11, 구현 웨이브 대기)**: GSC 실측 쿼리를 "데이터로 답하는 질문+답 블록"으로 전 콘텐츠 유형 커버. 헌장 6불변식(답 없는 질문 금지·엔티티 불변·변형어 2~3 직조 규칙 등) + 1층 intent_queue 탐지기 + 2층 유형별 Q템플릿(4갈래 에이전트 진단: 필드·갭·마운트·게이트 전수) + 웨이브 0~5. ⚠️핵심 갭: "best"는 실랭킹 필드 있을 때만·lineage는 영화단위(인물 노미네이션 금지)·edition_year≠film_year. **인텐트/Q&A/스니펫 세션은 여기서 시작.**
