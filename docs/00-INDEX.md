...
- `HANDOFF-방법론-독스.md` (루트) — **방법론 독스 허브 "The Method Docs" (P0~P4 구현 완료·SHIPPED 2026-07-12, 배포는 워처 정지 상태로 원우 대기)**: `/methodology`를 독스 사이트(상단 8카테고리 + 좌측 네비 + `/methodology/[slug]` 37문서)로 확장. 구성논리 전면 공개(영화 선정·figure 선정·TakeScore·lineage 가중·kinship/counterpoint 계산·credits WR·where-to·리셉션 사다리·locations 2패스 등), 공개 수위 3티어 헌장(§8) + 불변식 12(§9: 앵커 6종 보존·금지어 grep·Cinecodex 명칭 금지) + 페이지 연동 맵(§12) + 페이즈 P0~P5. 콘텐츠는 `lib/docs/content/*.ts` 마크다운-in-TS(워처 범위), 렌더러는 커스텀 결정론(라이브러리 0). **표준 템플릿 샘플: `site_content/METHODOLOGY_DOCS_SAMPLE_CONNECTIONS.md`(kinship 완성본).** 방법론 공개/독스 세션은 여기서 시작.
