...
- `docs/PLAN-home-v8-rotation.md` — **홈 v8 "살아있는 진열장" (Phase1·2 SHIPPED 2026-07-11)**: 홈 프리징 근본원인(home_v2_bundle 결정론 정렬)을 3층 시드 로테이션으로 해체 — `home_v2_bundle_v3(p_seed)` md5 샘플링(시간 시드) + `home_daily_exhibits`(일 시드, 오늘의 영화/질문/장소/재평가/오독/반론쌍 6타일) + 방문별 클라이언트 오프셋. 전 영화 카드에 `Tm {U}` TakeScore 칩(cinecodex.scores SQL 조인, 랭크 금지). 상단우측 Ask metatake AI 제거. Surprise 히어로 구조 불변. **실행 AI는 이 문서만 읽으면 됨**(사양·풀 크기·QA 체크리스트·함정 10종·파일맵 완비). 마이그 0070.
...
- `sentence-engine/` — **LLM-free sentence layer — MASS PRODUCTION SHIPPED 2026-07-11**. `film_sentences` = **454,555 entity-linked sentences, 12 patterns (A–I + J location, L trope, M frame), 6,713 films (96%)**, all values stored as uuid FKs for live links; + **kin index** `film_kinship` (27,593 pairs, cos+tfidf+rarity 0–100, replaces dead affinity score) + fanout stats (migration `0061_sentence_engine.sql`, ⚠️ manual commit). v1 factual template style = canonical (원우, 2026-07-11); v2 WOW doc = reference. Ops canonical: `sentence-engine/MASS-PRODUCTION.md` (gates·timeout runbook·refresh); start at `sentence-engine/README.md`. **Render surfaces Tier 1 SHIPPED 2026-07-11 (live): film "Did you know" module (`components/FilmSentences.tsx`) · SentenceTicker home+/room · map kin edge-weights (/map Films) · GraphCaptions. RPCs 0062 + map kin 0063.** Log + deferred items: `docs/WORKORDER-sentence-surfaces.md`.
