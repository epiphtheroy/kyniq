...
## 11. 개정 로그
- **2026-07-03 (3차 — v2 전면 리뉴얼)** **실데이터(702편) 투입 후 오너 판정("4단 마지막 단 무의미·연계 약함·사용성 낮음")으로 §1의 "4단 셸 불변식"을 공식 폐기하고 v2 「데일리 루프 셸」로 교체.** 결정 과정: 병렬 감사 4에이전트 → 설계안 3개 경쟁(데일리 루프/2단 미니멀/포스터 퍼스트) → 저지 판정(승자 데일리 루프 35/40 + 패자 이식 11건) → 병렬 구현 4에이전트 → 3렌즈 리뷰 26에이전트(확정 22결함 전수 수정). **v2 확정 구조:** ①레일(9항목·3그룹: 오늘/자산/기록실 — desk는 /room에 흡수(redirect), pair·공개프로필은 ⌘K/아바타로) ②본문(max 880px) ③인스펙터=온디맨드 슬라이드오버(<900px 바텀시트, select 시에만 열림, 요약은 앱바 버튼, 라우트 이동 시 자동 닫힘+요약 path 스코프) — 티커·라이브피드·시스템카드 폐지. **홈 = "오늘"**: 퀵 기록(검색→반별점 원스트로크) → 오늘의 한 편(가용 우선+적합도, 담기/봤어요/다른 한 편) → 최근 본 스트립 → 다음 후보 6행(워치리스트와 동일 규칙=허브 프리뷰 계약) → 완파까지 N편 → 자산 한 줄(NAV·티어·관람·loved). **연계 3원칙:** 같은 행(FilmRow)·같은 인스펙터(RecInsp 3카드: 왜/Cinecodex/바로하기)·같은 mutation(useRoomActions). 숫자 예산(모듈 0~3·행 1~2), 금융 카피·λ다이얼·Δ+0·별자리·KPI스트립·U/S 노출 폐지. 컬렉션=4컬럼·최근순 기본·50행 페이지네이션·1000행 캡 청크 페치, 워치리스트=담아둔/추천 2섹션, 기록=퀵레이트+그리드+닮은 결. 불변식(비섞임·위험색·no-fake-data·설명가능·RPC 무변경) 전부 보존 — CinecodexCard가 인스펙터 내 정본. §6의 페이지별 구 사양은 v1 기록으로 유지(구현은 v2가 정본).
