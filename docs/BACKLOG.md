...
- 🟡 **SQL 문장생성 엔진 — 대량생산 SHIPPED 2026-07-11, 렌더 표면만 대기.** `film_sentences` **454,555문장·12패턴·6,713편(96%)** 프로덕션 적재(마이그 0061) + **kin 지수**(`film_kinship` 27,593쌍, cos40+tfidf25+희귀도35=0~100, 죽은 score 대체) + fanout 통계 2종. 모든 값 엔티티 컬럼(uuid FK)로 저장 → 링크 렌더 가능; `(film_id, salience desc)` 인덱스로 top-k 1쿼리. 원우 결정: **v1 사실형 템플릿이 정본**(v2 WOW 수사는 참조용, 계산만 데이터로 보존). 신규 패턴 3종(J 촬영지·L 트로프·M 프레임) 추가. **정본 운영문서: `sentence-engine/MASS-PRODUCTION.md`**. **렌더 표면 Tier 1 SHIPPED 2026-07-11(라이브 검증): film "Did you know" 모듈 + SentenceTicker(홈·/room) + 맵 kin 엣지가중(/map Films) + GraphCaptions; RPC 0062·map kin 0063.** 로그·보류항목 `docs/WORKORDER-sentence-surfaces.md`. 보류(Tier 2): map_film_overview kin·MapExplorer 캡션. *(Tier 1 완료; ✅)*
