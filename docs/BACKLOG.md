...
- ✅ **DONE — Embedding Fantasia · SQL 문장층 (전 범위 SHIPPED 2026-07-11).** `film_sentences` 466,974행·13패턴 + kin 지수 + 표면 전 전개(film/director/theorist/trope/figure/lineage/genre 판타지아·홈+room 티커·/map 4뷰 레일·kin 엣지가중·N_question 훅). **정본: 루트 `HANDOFF-임베딩판타지아-문장층.md`** (불변식·함정·인제스트 접점·종결 장부). 잔여 0건; 파킹(Tier 2, 지시 시에만): OG 훅 문장·Surprise 모드·TV lower-third. 신규 영화 인제스트 시 stats→kinship→패턴 멱등 재실행 (RUNBOOK Stage 18).
