#!/usr/bin/env python3
"""director-profile-batch — submit & fetch Portrait+Who's-Next via Anthropic Message Batches (~50% off).
Pairs with director-profile-gen.py (Opus). Stores the FULL parsed object {slug, portrait, next}.

  1) python3 director-profile-gen.py --emit-requests --all --min-films 3 --out worker/director-profile-all
  2) python3 director-profile-batch.py submit --out worker/director-profile-all
  3) python3 director-profile-batch.py fetch  --out worker/director-profile-all   (re-run until ended)
"""
...
OUT = args[args.index("--out") + 1] if "--out" in args else "director-profile-all"
...
def submit():
    ...
    if not os.path.exists(reqpath): sys.exit(f"missing {reqpath} — run director-profile-gen.py --emit-requests")
    ...
    if ids:
        ...
        print(f"✅ {len(ids)} batch(es) → {OUT}.batchids.txt\n   fetch: python3 director-profile-batch.py fetch --out {OUT}")
...
def fetch():
    ...
    for bid in ids:
        ...
        for line in body.splitlines():
            ...
            fh.write(json.dumps({"slug": cid, "portrait": d.get("portrait", ""), "next": d.get("next", [])}, ensure_ascii=False) + "\n"); fh.flush()
            done.add(cid); got += 1
    fh.close()
...
if __name__ == "__main__":
    ...
    else: sys.exit("usage: director-profile-batch.py [submit|fetch] --out worker/director-profile-all")
