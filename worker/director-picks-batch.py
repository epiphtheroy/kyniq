#!/usr/bin/env python3
"""director-picks-batch — submit & fetch 'Where to Start' via Anthropic Message Batches (~50% off).
Pairs with director-picks-gen.py (Opus). Stores {slug, picks:[{slug,label,reason}]} (raw model picks;
slugs are validated against the DB filmography at LOAD time).

  1) python3 director-picks-gen.py --emit-requests --all --min-films 3 --out worker/director-picks-all
  2) python3 director-picks-batch.py submit --out worker/director-picks-all
  3) python3 director-picks-batch.py fetch  --out worker/director-picks-all   (re-run until ended)
"""
...
OUT = args[args.index("--out") + 1] if "--out" in args else "director-picks-all"
CHUNK = argf("--chunk", 1000)
PRICE_IN, PRICE_OUT = 15.0, 75.0          # Opus; batch = 50%
...
def submit():
    ...
    if not os.path.exists(reqpath): sys.exit(f"missing {reqpath} — run director-picks-gen.py --emit-requests")
    ...
    for i in range(0, len(reqs), CHUNK):
        ...
        bid = json.loads(tx).get("id"); ids.append(bid); print(f"  ✓ batch {bid} ({len(part)})"); time.sleep(1)
    if ids:
        with open(f"{OUT}.batchids.txt", "a", encoding="utf-8") as f: f.write("\n".join(ids) + "\n")
        print(f"✅ {len(ids)} batch(es) → {OUT}.batchids.txt\n   fetch: python3 director-picks-batch.py fetch --out {OUT}")
    ...
    for bid in ids:
        ...
        for line in body.splitlines():
            ...
            try:
                ...
                err += 1; print(f"    ! {cid}: parse fail"); continue
            fh.write(json.dumps({"slug": cid, "picks": d.get("picks", [])}, ensure_ascii=False) + "\n"); fh.flush()
            done.add(cid); got += 1
    fh.close()
    cost = (tin / 1e6 * PRICE_IN + tout / 1e6 * PRICE_OUT) * 0.5
    print(f"\n✅ fetched {got} directors · batch cost ${cost:,.2f} · errors {err} · total {len(done)}")
    if pend:
...
if __name__ == "__main__":
    ...
    else: sys.exit("usage: director-picks-batch.py [submit|fetch] --out worker/director-picks-all")
