...
interface PublisherConfig {
  ...
  jitter_min_minutes: number;
  jitter_max_minutes: number;
  answer_delay_minutes: number;
  contribution_delay_minutes: number;
...
async function loadPublisherConfig(supabase: SupabaseClient): Promise<PublisherConfig> {
  ...
  return { ...DEFAULT_CONFIG, ...(data.value as Partial<PublisherConfig>) };
}
...
async function getEffectiveDailyCap(
  ...
  const daysSinceFirst = Math.floor(
    (Date.now() - firstPublish.getTime()) / (24 * 60 * 60 * 1000)
  );
  ...
  return Math.floor(
    config.ramp_start_cap + progress * (config.daily_publish_cap - config.ramp_start_cap)
  );
}

// ── Today's publish count ─────────────────────────────────────────

async function getTodayPublishCount(supabase: SupabaseClient): Promise<number> {
...
export async function runPublisherCycle(supabase: SupabaseClient): Promise<{
  published: number;
  skipped: number;
