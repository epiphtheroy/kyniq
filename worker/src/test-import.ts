...
async function main() {
  ...
  for (const line of lines) {
    ...
    const resolvedTitle = (movie.title as string) ?? title;
    ...
    const { error: upsertErr } = await supabase.from("films").upsert(
      {
        tmdb_id: tmdbId,
        title: resolvedTitle,
        original_title: (movie.original_title as string) ?? resolvedTitle,
        year,
  ...
  for (const f of pipelineFilms ?? []) {
    console.log(
      `  ${f.pipeline_status.padEnd(12)} ${f.questions_published}/${f.questions_target}  ${f.title} (${f.year}) — ${f.director}`
    );
  }
  console.log("─".repeat(80));
