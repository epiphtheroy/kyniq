...
async function callGemini(model, prompt, systemPrompt, jsonMode = false) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        try {
            ...
            if (attempt < maxRetries) {
                ...
                console.log(`[gemini] Network error (${networkErr.message}) — retrying in ${retryAfter}s (attempt ${attempt + 1}/${maxRetries})`);
                await new Promise((r) => setTimeout(r, retryAfter * 1000));
                continue;
            }
            throw new Error(`Gemini network error after ${maxRetries} retries: ${networkErr.message}`);
        }
        if (res.ok) {
...
async function callOpenAI(model, prompt, systemPrompt, jsonMode = false) {
    ...
    const body = { model, messages, temperature: 0.4, max_tokens: 4096 };
    if (jsonMode)
...
async function callModel(config, prompt, systemPrompt, jsonMode = false) {
    if (config.provider === "openai")
        return callOpenAI(config.model, prompt, systemPrompt, jsonMode);
...
// ── Heartbeat helper ──────────────────────────────────────────────
export async function writeHeartbeat(supabase, workerId, status, details, jobId, questionsProcessed, costSoFar) {
    const row = {
        worker_id: workerId,
        status,
    ...
    if (questionsProcessed !== undefined)
        row.questions_processed = questionsProcessed;
    if (costSoFar !== undefined)
        row.cost_so_far = costSoFar;
    await supabase
        .from("agent_activity")
        .upsert(row, { onConflict: "worker_id" });
}
async function updateJobStep(supabase, jobId, step, questionsDone) {
    const update = {
        current_step: step,
        updated_at: new Date().toISOString(),
    };
    if (questionsDone !== undefined)
        update.questions_done = questionsDone;
    await supabase.from("jobs").update(update).eq("id", jobId);
}
// ── Dossier Builder (§1 — once per film, cached) ─────────────────
async function buildOrGetDossier(supabase, film, dossierConfig, result) {
    // Check cache
...
Rule: Do not invent anything. If you cannot ground a detail in reality or broad consensus, omit it.`;
    ...
    if (!parsed || typeof parsed !== "object" || !("verified_facts" in parsed)) {
...
async function verifyAndFix(draftBody, dossierJson, film, question, verifierConfig, result) {
    let body = draftBody;
...
{"critical_errors":[{"claim":"...", "issue":"fabricated_detail|factual_error|ungrounded", "fix_suggestion":"..."}],
 ...
 "confidence_score": 0.0}`;
        ...
        result.total_tokens += resp.tokensUsed.total;
        const parsed = extractJSON(resp.text);
        const verify = parsed && "confidence_score" in parsed
            ? parsed
...
function codePreChecks(draft, voice) {
    const failures = [];
    if (!draft.tldr || draft.tldr.length < 10)
        failures.push("Missing or too short TL;DR");
    if (!draft.facts_used || draft.facts_used.length === 0)
        failures.push("No facts_used");
    ...
    if (bandMatch) {
        ...
        if (wordCount < minWords)
            failures.push(`Body too short: ${wordCount} words (band: ${voice.length_band})`);
    }
    return failures;
}
async function scoreWithRubric(draft, scorerConfig, result) {
...
// ── Main graph ────────────────────────────────────────────────────
export async function processJob(jobId, supabase, workerId) {
    const result = {
        questions_created: 0,
    ...
    const { data: film, error: filmErr } = await supabase
        ...
        .single();
    if (filmErr || !film)
        throw new Error(`Film not found: ${j.film_id}`);
    const f = film;
    // 3. Load config
    ...
    const routerConfig = (configMap.get("model_router") ?? {});
    const voices = (configMap.get("personas") ?? []);
    const gateConfig = (configMap.get("gate_threshold") ?? { default: 0.80 });
    const threshold = j.params.threshold ?? gateConfig.default;
  ...
  "hook":"why this baits a click", "pitch":"...", "dossier_refs":["relevant keys"]}]`;
    ...
    for (let i = 0; i < plan.length; i++) {
        ...
        try {
            ...
            // ── DRAFT (§3) ──────────────────────────────────────────────
            await writeHeartbeat(supabase, workerId, "running", `drafting ${i + 1}/${plan.length} for "${f.title}" [${voice.codename}]`, jobId, result.questions_created, result.total_cost_usd);
...
{"tldr":"...", "body":"...", "facts_used":["dossier keys"], "evidence_used":["dossier cinematic_evidence keys"],
 "voice_id":"${voice.id}"}`;
            ...
            if (draftParsed && typeof draftParsed === "object" && "body" in draftParsed) {
                draft = draftParsed;
                // Ensure tldr exists
                if (!draft.tldr)
                    draft.tldr = draft.body.split(/\n\n/)[0]?.slice(0, 200) ?? "";
            }
            else {
            ...
            if (preCheckFails.length > 0) {
                ...
                if (revisedParsed && typeof revisedParsed === "object" && "body" in revisedParsed) {
                    draft = revisedParsed;
                    if (!draft.tldr)
                        draft.tldr = draft.body.split(/\n\n/)[0]?.slice(0, 200) ?? "";
                }
            ...
            // ── VERIFY + FIX LOOP (§4) ──────────────────────────────────
            await writeHeartbeat(supabase, workerId, "running", `verifying ${i + 1}/${plan.length} for "${f.title}"`, jobId, result.questions_created, result.total_cost_usd);
            await updateJobStep(supabase, jobId, "verifying", i);
            const { finalBody, verify, retries: verifyRetries } = await verifyAndFix(draft.body, dossierJson, f, item.question, verifierConfig, result);
            ...
            if (status === "approved") {
                try {
                    const media = await curateMedia(supabase, "question", qRow.id, f.tmdb_id, f.title, f.year ?? null, f.director ?? undefined, item.question);
