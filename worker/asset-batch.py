#!/usr/bin/env python3
"""asset-batch — submit & fetch "Why watch" dossiers via Anthropic Message Batches (≈50% off).
  1) python3 asset-gen.py --emit-requests --all --out asset-all  → asset-all.requests.jsonl
  2) python3 asset-batch.py submit --out asset-all  [--chunk 1000]
  3) python3 asset-batch.py fetch  --out asset-all   → asset-all.jsonl ({slug, lenses})  (re-run until ended)
"""
...
OUT = args[args.index("--out") + 1] if "--out" in args else "asset-all"
CHUNK = argf("--chunk", 1000)
PRICE_IN, PRICE_OUT = 15.0, 75.0          # Opus 4.8; batch = 50%
...
def submit():
    ...
    if not os.path.exists(reqpath): sys.exit(f"missing {reqpath} — run asset-gen.py --emit-requests --all --out {OUT}")
    ...
    if ids:
        ...
        print(f"✅ {len(ids)} batch(es) → {OUT}.batchids.txt\n   fetch: python3 asset-batch.py fetch --out {OUT}")
...
def fetch():
    ...
    tin = tout = tcache = got = err = 0; pend = []
    for bid in ids:
        ...
        for line in body.splitlines():
            ...
            tin += u.get("input_tokens", 0); tout += u.get("output_tokens", 0); tcache += u.get("cache_read_input_tokens", 0)
            ...
            fh.write(json.dumps({"slug": cid, "lenses": lenses}, ensure_ascii=False) + "\n"); fh.flush()
            done.add(cid); got += 1
    fh.close()
    cost = (tin / 1e6 * PRICE_IN + tout / 1e6 * PRICE_OUT) * 0.5
    print(f"\n✅ fetched {got} films · batch cost ${cost:,.2f} (in {tin}, cache-read {tcache}, out {tout}) · errors {err} · total {len(done)}")
    if pend:
...
if __name__ == "__main__":
    ...
    else: sys.exit("usage: asset-batch.py [submit|fetch] --out asset-all")
