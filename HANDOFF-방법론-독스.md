...
- 2026-07-12 (v3, 최종 감수 패스) — **보수적 톤 매스킹 배포**. 13그룹(연관 문서군) 편집+검증 26에이전트: 마케팅·과장·전투적 어휘 제거("the weapon"·"strip-mined"·"the whole game"·"welcome the fight"·"pollution"·"elite" 등), 문서 간 중복 절단+상호링크(what-ai-does↔ai-disclosure·how-a-page-is-made↔editorial-responsibility·never-blend는 what-takescore-ignores 단독 소유), 모순 정리(광고: 영구 금지선="작품 내부 구매"로 재획정·신설 firewall 조항이 정본 / collaborate "non-commercial"→"independent"), **교수 괄호 문장 삭제**, kin_edges(46,440)↔kinship pairs(27,593) 라벨 혼동 2곳 수정, 오링크 2곳(frameworks figure→/methodology/figures·sentences readings→/strong-misreadings), search 12종 열거 정합. TSX: /editor 메타·/about 편집기준 → 데스크 프레이밍. 검증: 46문서 렌더+링크+금지어("professor"·현직 포함) 전수 클린·프로덕션 빌드.
- 2026-07-12 (v2, 원우 20개 리뷰 반영) — **개정 배포**. 문서 37→**46**(신규 10: the-daily·metatake-tv·my-room·my-films·import·save-and-share·theory-explorer·collaborate·bots-and-crawlers·sources-we-monitor), 신규 카테고리 "Your Cinema", 렌더러 `:::details` 토글 추가(sources-we-monitor 4토글). 반영: **#10 수치 위계 정정**(1,935 read·6,701 scored·~5,975 lineage·6,975 catalogue — 전 문서 `{{n:films}}` 오용 교정, tiers 3티어 타일), **#11 분포**(TakeScore median~36·8–57·Value58 / lineage standing median~33·max~99·70개국), #1 미션(아는만큼보인다), #5/#6 편집"데스크"+검수 파이프라인 스케일, #7 봇, #8 협업, #12 바이라인+편집일+Article dateModified, #13 광고 firewall, #14 수익화, #15 홍상수 34편·인간적, #16 현직 삭제(methodology+/about+/editor), #17 WY.Heo 실존인물+사이트 전역 "to. WY. Heo"(TowCard·director·lineage·for-w-heo 디스플레이, 슬러그 유지), #18 대담한 AI 담론, #19 소스 토글, #20 `MethodologyBadge`(Provenance 경유 사이트전역 "?"). 검증: 전 46문서 tsc+렌더+**프로덕션 빌드 통과**, 금지어 0, 내부링크 47슬러그 전수 검증, 18개 변경문서 어드버세리얼 검증 워크플로. ⚠️ /methodology 앵커는 #corrections·#index만 잔존참조(둘 다 허브 보존).
- 2026-07-12 — **P0~P4 구현 완료(SHIPPED, 배포 대기)**. 셸: `app/methodology/layout.tsx`·`methodology.css`·`[slug]/page.tsx`, `lib/docs/{registry,md,content/*}.ts`, `components/docs/{DocsSidebar,DocsTopTabs,DocsPager}.tsx`. 허브 `page.tsx`에 카드그리드 + 앵커 6종 보존한 채 딥링크 6개 추가. 문서 37편 전부 저술(kinship=수작업, 나머지 36편=워크플로 author→adversarial verify 70 에이전트, 0 에러). 사이트맵 `methodology.xml` 자식 3파일 배선, `llms.txt` 레지스트리 연동 재작성. **검증**: 전 파일 tsc 클린, 렌더러 유닛테스트(kinship 14/14) + 전 36문서 컴파일+렌더 스모크테스트 통과(avg 878단어·타일26·표17·토큰누출0·`<script>`0·표/인용 균형), 금지어 grep 0, escaped-backtick 버그 1건(credits.ts) 발견·수정. 함정 기록: **워크플로 author 에이전트가 닫는 백틱을 `\`;`로 과잉이스케이프 → 문자수 스캔(==2)은 통과하나 template literal 미종료(TS1160). 향후 content 저술 검증은 반드시 실제 tsc 컴파일로(문자수 아님).** **배포 완료(2026-07-12 07:26 push, commit 44df832; 프로덕션 라이브 검증: /methodology/kinship 200·앵커 6종 보존·서브독 200).** P5(/ko)는 별도 세션.
