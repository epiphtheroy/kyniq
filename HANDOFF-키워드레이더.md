...
>
> **🎯 방향 A 전환 — 개인 창작자 발굴 (2026-07-12 라이브, commit 참조): 라이브 결과 수집물의 63%(556/1000)가 대형 매체였다.** 목적을 "매체 모니터"에서 **"개인 창작자 발굴(관계·백링크용)"**로 전환. 변경: (1) **마이그 0084** `radar_items.author_kind`(individual|institution) + `common.classify_author`(도메인 블록리스트 §16 참조) — **/admin/radar 기본 뷰는 institution 숨김(개인만)**, People/All/Institutions 토글. (2) **Letterboxd 추가**(`poll_letterboxd.py` + `add_letterboxd.py`) — 개인 시네필의 본진, 오너 큐레이션 멤버 RSS 풀(스크래핑은 robots AI차단+HTML 403이라 불가; 큐레이션이 관계도구로도 정답), 구조화된 `filmTitle/filmYear`로 **metatake 전체 영화 코퍼스(entities.json ~7k) 정확매칭**(연도 ±1 허용, 오탐 0)→**직접 `/film/{slug}` 링크**. (3) **GDELT 비활성**(순수 뉴스/기관 — 코드 유지, watch.sh에서 뺌), 기관 피드 소스 20개 비활성화. (4) **각 카드에 "→ metatake" 액션 링크**(Letterboxd=직링크, 그외=`/search?q=`) — 발굴→마케팅 액션(그 창작자에게 줄 링크) 연결. (5) Bluesky **DID→핸들/이름 해석**(public AppView getProfile, 캐시)로 "누구인지" 표시. **⚠️ 전략(§honest): 좋아요 스팸 ❌ — "링크할 이유(우리 영화 페이지)"를 줘 백링크/리퍼럴 유도가 실제 트래픽 레버. 레이더는 소수 고가치 창작자 관계구축 도구.** 첫주 튜닝: firehose 비영화 오탐(키워드 조정)·wordpress 발견 풀 개인/기관 재점검.
