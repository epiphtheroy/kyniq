...
인명의 뜻·유래를 설명하는 짧은 글. **한자·원어 병기가 가장 자연스러운 층**이다(예: 侯孝賢).`,
  leads: `## 이 묶음의 성격 — 인바이테이션(초대문), 첫 번역
한 편으로 독자를 초대하는 긴 산문이다. 사이트의 간판 글이고, **앱 영화 화면에서 '초대'로 렌더된다.**
- 기존 한국어가 없다(prev 없음). 영어 원문만 보고 처음부터 한국어로 써라.
- 첫 문장이 특히 중요하다. **덱(Tonight)에서는 첫 문장만 잘려 나가 포스터 아래 한 줄로 선다** —
  그 한 줄이 혼자 서야 하고, 그 뒤에 이어질 문장을 전제해서는 안 된다.
- 금지: 130자짜리 선행 관형절, 대시 남용, 명사화 나열, 합쇼체 혼입.
- 호흡은 길되 문장은 끊어라. 한 단락 안에서 긴 문장과 짧은 문장을 번갈아.
- 감독·배우 인명은 한글 음역, 영화 제목은 한국 개봉명이 확실할 때만 그것으로(불확실하면 원제 유지).
  연도·수치는 원문 그대로.`,
  invitation_new: `## 이 묶음의 성격 — 인바이테이션(초대문), 첫 번역
위 \`leads\`와 같은 층이다. 한 편으로 독자를 초대하는 긴 산문이며, 기존 한국어는 없다.
- 첫 문장은 덱에서 홀로 잘려 나가므로 그 자체로 완결되어야 한다.
- 금지: 130자짜리 선행 관형절, 대시 남용, 명사화 나열, 합쇼체 혼입.
- 호흡은 길되 문장은 끊어라. 인명은 한글 음역, 연도·수치는 원문 그대로.`,
