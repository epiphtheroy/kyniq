...
**Metatake의 자산(콘텐츠·기술)을 지렛대로 한 B2B 아웃리치 전 과정 — 후보 발굴(서치 봇) → 컨택 DB → 오퍼 매칭 → 초안 생성(룰 엔진) → 사람 승인 → Gmail 발송 → 응답 관리(룰베이스 자동응답 초안) → 이력·지표 — 을 한 화면에서 시작해 세부 페이지로 파고들 수 있게 만든 오너 전용 CRM.**
