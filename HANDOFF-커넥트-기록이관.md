...
## §0.5 웹 `/me/import`에 이식 완료 (2026-08-05) — ✅프로덕션

오너 제보: **"imdb 누르니까 작동이 안되더라구요."** 맞았다. 웹의 타일은 누르는 순간
**파일 선택창**을 열었는데, IMDb는 파일을 그 자리에서 주지 않는다 — **요청하면 큐에
넣고 몇 분 뒤 `imdb.com/exports`에서 수령**한다(§1 실사 그대로). 즉 사용자가 가질 수
없는 것을 요구하는 버튼이었다. 아이폰에서는 아예 **죽은 버튼**이었다: 사파리는
`display:none` 입력에 대한 프로그램적 `.click()`으로 피커를 열지 않는다.

**§0-2("앱은 길을 깔고")를 웹에도 적용했다.** 앱의 가이드 시트 구조를 그대로:

| 이식한 것 | 어디에 |
|---|---|
| 서비스별 **3단계 실버튼 가이드** + 내보내기 페이지 + (큐잉 서비스는) **수령 페이지** | `lib/import/connectors.ts` — 앱 `mobile/src/lib/connect.ts`에서 이식(문구 SSOT는 §1) |
| 타일 → 가이드 → **마지막에** 파일 선택 | `components/ImportWizard.tsx` |
| **복귀 배너**("IMDb가 준비 중입니다" / "파일 받으셨나요?") · 1시간 TTL | 같은 파일 + `localStorage: mt-import-awaiting`. 앱의 AppState 대신 `focus`·`visibilitychange` |
| 커넥터별 `accept` 좁히기(IMDb=`.csv`) | 아이폰 파일 앱에서 엉뚱한 파일만 보이던 문제 |

**웹에서만 드러난 것 3가지**

1. 🚨 **`display:none` 파일 입력은 사파리에서 안 열린다.** 시각적으로만 숨기고
   (`clip-path`) 렌더는 유지해야 한다. 아이폰에서 타일이 죽어 보인 진짜 이유.
2. **401을 "파일이 이상하다"로 번역하고 있었다.** 세션이 끊기면
   "Couldn't read that. Try pasting a plain list of titles." — 읽는 사람의 목록을
   탓하는 문구. 이제 로그아웃 상태를 말하고 새 탭 로그인 링크를 준다(입력은 보존).
3. **`parseFile`이 파일명을 믿고 있었다.** 안드로이드 문서 제공자는 확장자 없는
   표시 이름을 준다(패리티 원장의 "임포트 파일명에 확장자 없으면 CSV 판정 실패").
   확장자 없는 워크북은 **Letterboxd ZIP으로 오독**됐다 — 둘 다 "PK"로 시작한다.
   이제 **아카이브를 열어 엔트리 목록을 읽는다**(`xl/`=워크북, `watched.csv`=익스포트).
   TSV도 어떤 휴리스틱에도 안 걸리고 있었다. **서버 수정이라 웹·iOS·안드로이드가 동시에 고쳐진다.**

**덤으로 찾은 구멍**: `ratings.csv`에만 있는 영화는 파싱하고도 버려졌다 — 평점만 든
ZIP은 0편을 임포트하고 아무 경고도 내지 않았다. 이제 포함한다.

검증: 생성 픽스처 4종을 **정상 이름 / 확장자 없는 이름** 양쪽으로 — IMDb CSV·워크북·
Letterboxd ZIP·TSV 전부 2/2행, 4파일 완전 익스포트는 4행(다이어리 영화 1회만).
⚠️ **로그인 상태의 프로덕션 E2E는 여전히 미검증**(에이전트에 자격증명 없음).

**안드로이드**: 서버 수정이 그대로 적용되고, 추가로 피커가 **파일을 볼 수 있게**
MIME 별칭을 넓혔다(`application/x-zip-compressed`·`application/vnd.ms-excel` 등 —
`mobile/src/lib/connect.ts`, 커밋 `6bd590e4`, 브랜치 `feat/discovery-feed`).
⚠️ 앱의 안드로이드 백/타임아웃 보강은 **`feat/discovery-feed`에만 있고 main에는 없다**
([[branch-discovery-feed-behind-main]]).
