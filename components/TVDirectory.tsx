...
// TVDirectory — the browse UI for every METATAKE TV watch list (/tv/lists).
// Axis filter tabs (from tv_directory_summary) + search + a paged card grid
// (tv_directory). Each card links to the standalone /tv/list/[slug] page.
...
export default function TVDirectory({ initial, initialSummary, initialTotal }: { initial: Item[]; initialSummary: Sum; initialTotal: number }) {
  ...
  // refetch on axis/search change (skip the very first render — SSR seeded it)
  useEffect(() => {
  ...
  const totalAll = initialSummary.reduce((s, x) => s + x.n, 0);
      ...
      <div className="tvw-wrap">
        ...
        <div className="tvdir-controls">
          ...
          <div className="tvdir-axes">
            ...
            {initialSummary.filter((s) => s.axis !== "manual").sort((a, b) => b.n - a.n).map((s) => (
