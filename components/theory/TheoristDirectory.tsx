 ...
 * modes (by films staged / A–Z), and an inline filter, over a grid that shows
 * each thinker with a one-line gloss of the lens they give cinema and the
 * number of films read in their light. The count is the value signal: it is
 * how many films stage what this thinker named.
 */
export type TheoristRow = { slug: string; name: string; blurb: string | null; n: number };

const sortKey = (s: string) => {
  // sort surnames: last token, ignoring a trailing "the ..." etc.
...
export default function TheoristDirectory({ rows }: { rows: TheoristRow[] }) {
  ...
  return (
    <div>
      ...
      <style>{`
        ...
        .thx-card{display:flex;flex-direction:column;gap:3px;padding:11px 0;border-bottom:1px solid var(--hairline);text-decoration:none;color:var(--ink)}
