...
export default function FilmRow({ f, onOpen, onKeep, onSeen, onDismiss }: {
  ...
  return (
    <div className="frow" onClick={onOpen}>
      ...
      {f.fit != null ? <div className="fit">{f.fit}<small>적합도</small></div> : <span />}
      <div className="fact" onClick={(e) => e.stopPropagation()}>
        {onKeep ? <span className={`fb${f.kept ? " done" : ""}`} title="담기" onClick={onKeep}><i className="ti ti-bookmark-plus" /></span> : null}
        {onSeen ? <span className="fb" title="봤어요" onClick={onSeen}><i className="ti ti-check" /></span> : null}
        {onDismiss ? <span className="fb" title="관심없음" onClick={onDismiss}><i className="ti ti-x" /></span> : null}
      </div>
    </div>
  );
}
