...
> SPEC **§6.11** (retention engine). The **Kyniqbot relevance fix (bug #4)** is written into
...
3. **Question + pitch** (the pitch as a standfirst), then **the reading** (canonical answer) as
   the hero, with the Kyniq Editorial byline. If no answer yet, show a graceful prompt — but the
...
4. **Kyniqbot YouTube relevance is broken.** The Stalker page shows completely unrelated clips
...
3. Fix **Kyniqbot relevance** + remove **duplicate home feeds**.
