-- map_film_ego — films-mode ego graph for /map.
-- 2026-07-04 (connections overhaul, follow-up decisions): versioned into repo +
-- counterpoint edges added (kind='counter', top-4 from entity_edges) alongside
-- next/recby (film_next) and like (film_affinities). Client colors: EntityGraph EDGE map.
-- Applied via migration map_film_ego_counterpoints.
...
AS $function$
  ...
  lik as (select fl.slug,fl.title from film_affinities fa join films fl on fl.id=fa.related_film_id
  ...
  edges as (
    select 'film:'||(select slug from cf) s,'film:'||slug t,'next' kind, true arrow from fwd
    union all select 'film:'||slug,'film:'||(select slug from cf),'recby', true from rev
    union all select 'film:'||(select slug from cf),'film:'||slug,'like', false from lik
    union all select 'film:'||(select slug from cf),'film:'||slug,'counter', false from cp
    union all select 'film:'||srcslug,'film:'||slug,'next', true from r2
  )
  select jsonb_build_object(
    ...
    'links',coalesce((select jsonb_agg(distinct jsonb_build_object('s',s,'t',t,'kind',kind,'arrow',arrow)) from edges),'[]'::jsonb));
$function$;
