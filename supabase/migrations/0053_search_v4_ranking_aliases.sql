-- 0053_search_v4_ranking_aliases.sql — search ranking upgrade + Korean/alt-title aliases
--
-- Fixes (probed live 2026-07-10):
--   1. "kurosawa" ranked essay titles (flat 0.85 prefix boost) above the director
--      entity (similarity('Akira Kurosawa','kurosawa') = 0.6). Two causes, two fixes:
--        a. word_similarity() — an exact word inside a longer name now scores 1.0
--           (applied to every name-ish field: director/theorist names, film titles,
--           film.director, tradition.theorist).
--        b. kind weights — entities people navigate to (film, director) outrank
--           derived content (readings, figures) at equal lexical strength.
--   2. Korean queries returned garbage ("쿠로사와" → nothing related). New
--      search_aliases table (backfilled from Wikidata ko labels + alt labels via
--      worker/ko-aliases.py) joins into the film and director legs, so
--      "기생충" → Parasite and "구로사와" → Akira Kurosawa. Alias matches carry
--      a 0.97 factor so the native-title match never outranks an equal title match.
--   3. Same film reachable via title AND alias produced duplicate rows → outer
--      dedupe keeps the best-scoring row per (kind, slug).
--
-- film_search v3: alias-aware too (film pickers accept Korean titles).
-- NOT done here (deliberate): unaccent() — live probes show trigram similarity
-- already survives diacritics ("amelie" → Amélie #1), and unaccent() in WHERE
-- would bypass the GIN trgm indexes (seq-scan on takes).

-- ---------------------------------------------------------------------------
-- search_aliases — alternative names per entity (Korean labels first)
-- ---------------------------------------------------------------------------
create table if not exists public.search_aliases (
  id bigint generated always as identity primary key,
  kind text not null check (kind in ('film','director','theorist')),
  slug text not null,
  alias text not null,
  lang text not null default 'ko',
  source text,                       -- 'wikidata-label' | 'wikidata-alt' | 'manual'
  created_at timestamptz not null default now(),
  unique (kind, slug, alias)
);
create index if not exists idx_search_aliases_alias_trgm
  on public.search_aliases using gin (alias gin_trgm_ops);
create index if not exists idx_search_aliases_kind_slug
  on public.search_aliases (kind, slug);
alter table public.search_aliases enable row level security;
drop policy if exists "search_aliases readable" on public.search_aliases;
create policy "search_aliases readable" on public.search_aliases
...
kw(kind, w) as (values
  ...
  ('lineage', 0.88), ('archetype', 0.85), ('figure', 0.80), ('reading', 0.78)
)
select d.kind, d.slug, d.film_slug, d.title, d.sub, d.poster, d.year, d.score, d.is_catalog
from (
  select r.*, (r.score * kw.w)::real as wscore,
         row_number() over (partition by r.kind, r.slug order by r.score * kw.w desc) as rnk
  from (

  -- films — title, original_title, director, aliases (Korean & alternative titles)
  ...
  union all
  -- directors (aliases: Korean names etc.)
  ...
  union all
  -- theorists — composite rows ("A / B", "A and B", "A, B": known table pollution)
  -- take a 0.6 penalty so the single canonical person outranks every pairing.
...
-- ---------------------------------------------------------------------------
-- film_search v3 — alias-aware (Korean titles in every film picker)
-- ---------------------------------------------------------------------------
...
as $$
  ...
  where (f.visible or f.slug not like 'tmdb-%')
    ...
    and (f.title ilike '%'||p_q||'%' or f.original_title ilike '%'||p_q||'%'
         or exists (select 1 from search_aliases a
                    where a.kind='film' and a.slug=f.slug and a.alias ilike '%'||p_q||'%'))
  order by (f.title ilike p_q||'%') desc, f.visible desc,
           greatest(
             similarity(f.title, p_q),
             coalesce((select max(similarity(a.alias, p_q)) from search_aliases a
                       where a.kind='film' and a.slug=f.slug), 0)) desc,
           f.title
