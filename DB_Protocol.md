...
**Version:** v2 (2026-06-28) · prior snapshot kept as `DB_Protocol_v1.md`
...
*   **Word Count (v2 — SHORTER):** Approximately **1,100 words** (English essay; acceptable range 1,000–1,250). Brevity is now a virtue — cut anything that merely decorates.
*   **Rhythm (v2 — BRISKER):** Keep the pulse light and quick. Vary sentence length deliberately and favour short, punchy sentences; avoid long subordinate pile-ups and triple-clause sentences. The reader should feel propelled, not steeped. When in doubt, end the sentence early.
*   **Wit Density (v2 — HIGHER):** A witty turn — a deflation, a surprising comparison, a dry aside — in **every paragraph, ideally more than one.** Wit is the engine of pace, not a garnish. Lane is funny on almost every line; match that frequency.
*   **Fact-Per-Beat Rule (v2 — CRITICAL, the signature of this version):** The essay must be **dense with concrete fact.** Aim for a factual datum in *almost every sentence*; at an absolute minimum, **in every two-sentence unit at least one sentence must carry hard, verifiable fact** — a name, date, number, place, role, runtime, award, language, source text, box-office or festival detail, etc. The DB's figures and the film's record are the raw supply; mine them generously. Two consecutive sentences of pure atmosphere or abstraction is a violation — break it with a fact. **Verify** any asserted fact (use web search if unsure); accuracy is non-negotiable, since fact is now load-bearing.
*   **Fact ≠ Citation (Authorial Voice still holds):** Facts are delivered as the critic's own knowledge, woven into the wit — never as "the database says," never as a table read-out, never with a score quoted as data. A fact and a DB-reference are different things: keep the facts, hide the machinery.
...
*   **Structure:** 7–8 tight paragraphs as designed in Phase 1.5 (fewer, shorter paragraphs are fine at the new length).
...
*   **v2 (2026-06-28)** — Style overhaul to the essay output. (1) Shorter: target dropped from ~1,600 to ~1,100 words. (2) Brisker rhythm: short, propulsive sentences. (3) Higher wit density: a witty turn in every paragraph. (4) **Fact-Per-Beat rule:** near-every sentence carries a concrete fact; minimum one hard fact per two-sentence unit; facts must be verified. Authorial Voice unchanged (facts yes, DB-machinery no). Prior version preserved as `DB_Protocol_v1.md`.
*   **v1 (2026-06-28)** — Initial DB Protocol. Mirrors the modified Master Protocol; replaces `source.md` with live MetaTake DB mining (strong misreadings, director material, connection graph). Essay target ~1,600 words.
