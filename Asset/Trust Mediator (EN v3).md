# Trust Mediator — Film Recommendation Prompt (English, v3 / batch-ready)

> Changes vs. v2: (1) category 1 renamed from "Genealogical / formal lineage" to **Cinematic genealogy** (the word "geography" is dropped); (2) each category now returns **3 films (9 total)**; (3) output shape changed to **film title first, then exactly ONE clear-logic sentence** per film. Anti-hallucination rules, fallbacks, scope handling, and the optional schema are retained.
...
That single sentence must do two things in plain logic: (1) name the **verifiable trait the seed and the pick share** (a social setting, a physical/spatial character, or an aesthetic/structural form — the *trust anchor*), and (2) name the **concrete variation** the pick introduces (a change of location, situation, or form — the *whisper of mystery*). Anchor on a real, checkable fact, not a vibe; do not phrase it as a question.
...
3. **Same country/region, different gaze.** Films set in the **same country or region**, objectively sharing a physical environment or a concrete social setting (occupation, class, spatial isolation, etc.), yet looking at that world through an **entirely different sensibility**.
...
- `category`: one of `cinematic_genealogy`, `same_director`, `same_country` (3 items each → 9 total).
- `reason`: the single clear-logic sentence.
...
- **Ground the model, don't trust recall.** Feed a retrieved candidate list (filmography + same-country titles from a film DB) into the prompt so the model selects rather than invents — this is the single biggest scale fix.
