...
>
> **⚠ 갱신(2026-06-14) — 형상 결정 뒤집힘.** 아래 §5.1·§5.5·§13(B)·백로그 #2의 "형상=독립
> 페이지 아님(섹션 SEO), 형상당 take 보강은 지금 안 함" 결정은 **폐기**되었다. 새 결정: **모든
> 형상이 독립 페이지** `/film/[slug]/figure/[fig-slug]`를 갖고, **형상당 렌즈·메타테이크가 다른
> take ≥3**을 두며, 그 위에 **로그인 사용자 기여 레이어**(take·figure 추가, 메타테이크 선택/제안)를
> 얹는다. 형상 페이지/기여에 관한 한 **`figure-page-design.md`가 최신·우선**이다.
...
- **형상 SEO(~~확정 B~~ 폐기 → figure-page-design.md):** 형상은 **독립 페이지**
  `/film/[slug]/figure/[fig-slug]`를 가진다. 영화 페이지의 형상 목록은 label을 그 페이지로 링크.
  thin-content 방어선은 "페이지로 안 만들기"가 아니라 **take ≥3(렌즈·메타테이크 다양) + 사용자
  기여 성장 + 조밀한 양방향 링크**. (과도기 take<3 형상은 단계적 색인.)
...
### 5.5 형상 페이지 `/film/[slug]/figure/[fig-slug]` (갱신)
**독립 페이지.** 형상 label(h1) + kind/영화 info + **영화적으로 구체적인 묘사**(건조·관찰) +
**Readings**(take 카드들 — 각 카드에 렌즈 배지 + 메타테이크 링크 + 해석문 전문) + Kin(횡단 친족)
+ **기여 CTA**. take 페이지의 `.mt-*` 디자인 재사용. 상세·스키마·기여 UI = `figure-page-design.md`.
...
- **Q&A 격리(확정 1).** 기존 133 질문+답변 — **별도 폴더/네임스페이스로 분리(폐기하되 비삭제)**.
  ...
  - **갱신(2026-06-14):** Q&A *UI*는 부활하지 않는다. 대신 Q&A의 **기여 메커니즘**(로그인 사용자가
    글을 더하고 모더레이션)을 **형상/테이크/메타테이크 척추에 흡수** — 형상 페이지에서 take 추가
    (메타테이크 선택 필수)·figure 추가. `/admin/review` 큐 패턴과 `contributions` 업보트 관행을
    재사용. 상세 = `figure-page-design.md` §7.
...
- (B) ~~형상 = 독립 페이지 아님, 구조화 섹션~~ **→ 폐기(2026-06-14).** 형상 = 독립 페이지
  `/film/[slug]/figure/[fig-slug]` + take ≥3(렌즈·메타테이크 다양) + 사용자 기여. (figure-page-design.md)
...
2. **형상당 take 보강 — 활성화(2026-06-14, ~~지금 안 함~~ 폐기).** 형상 페이지가 독립 페이지가
   되면서 **형상당 렌즈·메타테이크가 다른 take ≥3**이 요건이 됨. AI enrichment 워커
   (`figure-enrich`)로 take<3 형상을 보강 + 묘사 §5 기준 재작성. 사양 = `figure-page-design.md` §8.
