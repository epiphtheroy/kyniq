const body = `
# Tropes and the maturity arc

A trope on Metatake is a coded reading pattern — a way of reading a film that recurs across many films and, in recurring, loses the singularity it once had. Every trope is [computed](/tropes), not curated, but the last word belongs to a critic step rather than to a number. This page explains what a trope is, where each one sits on a maturity arc, and why a pattern only earns the name once it carries a genuine code.

## In numbers

> **{{n:tropes}}**
> named tropes
> each is a recurring reading pattern with an evocative name and a stated code

> **{{n:figures}}**
> figures behind them
> the concrete things films return to, from which the reading patterns are read
...
Every pattern sits somewhere on a single arc, read from two things at once: **recurrence** (how many distinct films share it) and **cohesion** (how tightly the readings actually resemble one another). The arc runs from the most singular to the most conventional.
...
| **Cliche** | Fully conventional — either the most widely shared patterns or ones that match a convention the culture already recognises. The most shared, and the least singular. |

The badge attached to each trope is nothing more than which band it falls in. It is a plain reflection of how many distinct films share the reading, sharpened by how tightly they cohere — not a quality score and not an editorial ranking.

## Proximity proposes, a critic decides
...
A few calls were not obvious. We hold a **minimum of two films**: a true pair, found across all of cinema, is already a discovery worth naming, while a single film's reading stays Noble. We **never force a split or a merge to hit a number** — a trope is whatever coheres. We do not break a coherent large cluster because it is large, nor glue weak ones together to look tidy; the count of tropes is emergent, not a target. And when two tropes turn out to name the same code under different words, a later pass merges them; when they only look alike, it keeps them apart and simply links them as related.
...
> Tropes sit under the same [corrections](/methodology#corrections) loop as everything else: if a pattern is mislabelled or a film plainly does not belong, tell us and we will fix the row.
`;
export default body;
