...
So each route is written by **Metatake AI** reading the whole filmography at once against a brief the desk sets: cover the way in, the peak, and the deep cut; choose only from films we actually hold; name the film; say specifically what it does and who it is for; never rank by box office. The desk writes that brief and answers for what it produces. Be clear about the gate, though, because it differs from the readings: a reading is checked draft by draft before it publishes, and these routes are not. They were generated in one batch and loaded as they came. That is a weaker guarantee, and it is why an occasional route reads better than it judges — and why the [corrections](/methodology#corrections) loop matters more here than anywhere else on the site. This is the same system that drafts the readings, pointed at a different question — not an editor working through a filmography by hand.
