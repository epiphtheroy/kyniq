...
So the judgement inside the letter is partly machine-computed, while the prose around it is a rule. That is why the letter comes from the desk rather than carrying a person's signature, as it once did. Nobody sat down and wrote it, and we would rather say so than lend a human name to a template.
