...
| Full nominee slates | We record winners and ranked members only — a Best Picture list holds the film that won, not the four it beat |
...
Two calls were not obvious. We chose to record **winners and ranked members, not nominees**, because nomination data is not uniformly public across bodies and eras — and we would rather under-claim than pad a page with a slate we cannot verify end to end. And we chose to treat mere **selection** as the lighter fact it is: being shown at a festival is not the same as winning it, and a bare selection is filed as such rather than counted as an honour.

We also lean, on purpose, against the centre. Pure prestige tilts hard toward Cannes and Hollywood, so the national honours are a deliberate counterweight, not an afterthought — a way of keeping the loudest circuits from drowning out a film whose recognition is rarer and further from the middle.

## Limits

A lineage list records where a film sits in cinema's public honours record — what it *won* and where it is *ranked*. It is not a measure of quality, and it says nothing about box office or audience ratings, which live where they belong and not in this layer. A handful of lists on the site are partial, and where that is true the page says so plainly — a canon can honestly read "994 of 1,000" rather than quietly resolving the last six to the wrong films. A short count here is a real count, not a rounded one.
