const body = `
# Close readings and catalog records

Not every film on Metatake gets the same kind of page. Some have been read closely — broken into figures, each carrying published readings. Most have not; they are catalog records that carry facts and a short editor's note. We keep the two kinds visibly different, and only one kind is offered to search engines. This page explains the two tiers, how a film moves from one to the other, and why we do not dress a stub up as an essay.
...
This matters because it keeps the promise honest in both directions. A catalog record cannot be quietly promoted to look like an essay without the essay actually existing; the figures have to be there. And a film that has been read closely cannot be left languishing as a stub — the trigger surfaces it automatically.

## What we decided, and why

A few calls here were deliberate.

**We index what we have read, and only that.** It is tempting to open the whole catalog to search engines — more pages, more surface. We decided against it. Thin, syndicated pages at scale read as doorway content, and they are, so we hold them back rather than pad them into something they are not.

**We keep the two templates visibly distinct.** A catalog record does not borrow the close-reading layout. It leads with the editor's digest, marks itself as a record, and demotes the raw synopsis to the bottom. The point is that a reader — and a crawler — can tell at a glance which kind of page they are on.

**Catalog records still carry real editorial signal.** Even unread, a film sits inside curated [lineage](/lineage) lists, gets recommended by films we *have* read, and may carry our own filming-location data. The digest draws only on those held facts, is bylined, and is stamped with the date its underlying data last changed — never back-dated, never invented.
...
The tier a film sits in reflects **where our reading has reached, not the film's worth**. A great film we simply have not written about yet lives as a catalog record, indistinguishable in that respect from a minor one. The number of read films grows as the writing does; it is not a ranking.

The digest on a catalog record is assembled from data, so it is only as complete as that data. Where we hold little — a film with no canon placement, no inbound recommendation, no location pins — the digest is short, and we would rather it be short than filled out with generic prose. And because promotion is automatic, a film can cross into the indexed tier the day its third figure is approved, which means the set of indexed pages is always moving.

---

> Tiering sits under the same [corrections](/methodology#corrections) loop as everything else: if a catalog record carries a fact that is wrong, tell us and we will fix the row.
`;
export default body;
