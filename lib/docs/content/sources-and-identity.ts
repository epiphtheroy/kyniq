...
Metatake is a mix of things we borrowed and things we made, filed against one shared spine. This page draws the line between the two: which facts about a film come from outside sources, which judgments are ours, and how every layer of the site is pinned to the same film so that a reading, a lineage membership, and a location pin can never drift onto different movies.
...
Everything that carries a judgment is our own work, made on the site and answerable to a named editor.
...
We anchor on TMDb **because it is open and shared**, not proprietary to us — which means a Metatake identity is one anyone can look up and check, and one that other film databases already speak. Borrowing an open identity keeps our judgments auditable against the outside world.
