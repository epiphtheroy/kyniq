...
## 12. AS-BUILT — 인라인 크레딧 전면 교정 (2026-07-18, 오너 "모두 진행" 승인)

**배경 정정:** 정찰 결과 P1·P2는 `fff238b`가 아니라 **`6953cd1`로 랜딩**했고, 뒤에 후속 4개(`17b5fc4`·`fcb93cb`·`bea17ad`·`baa4653`)가 붙은 뒤 **2026-07-18 08:38 `release: staging→main`으로 이미 main 병합·배포됨**(11-4의 "P1·P2만 배포"는 이 시점부로 과거). 그런데 §5로 이월됐던 **인라인 크레딧이 옛 문구 그대로 라이브**여서 이 패스로 교정.

**두 病:** ① 구 명칭 `Metatake Editorial`(개명 선언만 하고 실물 미교체) ② 하지 않는 인간 노동(`edited by [Yoon]`·`Curated, not algorithmic`·`— Yoon, Editor` 서명).

**적용 표준 어휘 (신설 V 포함):**
- **A**(AI 집필·해석) = `Written by Metatake AI, to a framework directed by [Yoon]` — 인비테이션·why-watch·강한오독·theory·next/start·theorist(+Wikidata 참조 명기)·concept(+related films computed)
- **C**(AI 계산) = `Computed by Metatake AI · rubric/method designed & calibrated by [Yoon]` — 커넥션엔진(카운터포인트·킨드레드·movies-like)·director takescore
- **B**(규칙·LLM-0) = `Composed by the Metatake method · no language model · designed by [Yoon]` — credits 산문·reception(`Compiled from cited sources`)·honors(`public award records`)·lineage(`public sources`)
- **V**(AI 조사·검증, 신설) = `Researched by Metatake AI — each location corroborated by independent sources and cited, single-source claims held back · geolocated by Metatake` — **촬영지**. 산출 스크립트(`movie-locations-project/movie_locations_llmsearch.py`)로 검증: LLM+웹서치 조사 → LLM verify + 규칙게이트(독립 도메인 ≥2) → 위치마다 URL 필수(없으면 drop) → verified만 배포·단독출처/보호DB 격리. 핀 산문(`scene_role`)은 gpt-4o-mini라 **"no language model"을 쓰지 않음**(D12 정직 해소).

**28파일 변경**(app/film·director 서브 8종·concept·theorist·credits·locations×3·lineage×2·movies-like·frame·q·reception·poetics 메타·Byline·TowCard + ko.ts 페어 7 + 신규키). 오너 짚은 6곳 전부 포함(to.W는 `rec_date`를 short 변형에도 노출). 검증: `i18n-audit` 누락 0·`tsc` 20=기준선(회귀 0).

**남은 것(계획대로 보류):** ⓐ `admin/review:101` `author_name` + `lib/pipeline.ts`(라이브 LLM 시스템 프롬프트, "You are Metatake Editorial…" — 이미 "There is no human editor after you"라 정직층과 일치) + `lib/lineage.ts:80` `editorial.name`(JSON-LD 데이터) = **P5**. ⓑ 공유 **`components/Provenance.tsx`**("Generated by the Metatake editorial method · editorial desk led by Yoon")는 구 시스템명·"edited by" 어느 病도 아니라 **미변경**(Byline은 `Metatake Editorial→AI` 교체함). ⓒ `/catalog/[seg]/[slug]`(D10, **인간 스프레드시트** 정의)는 별도 — /concept와 다름(혼동 주의). ⓓ R3(`director_next.rec_name` 오염)는 라이브 데이터 버그로 여전히 미결.
