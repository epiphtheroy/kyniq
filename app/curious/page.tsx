 ...
 * /blog 2026-07-07; /blog/curious 308s here). Index of every published
 * featured Q&A plus the desk essays. Reading happens on the film-scoped
 * pages (/film/[slug]/q/[qslug], /film/[slug]/[desk] — the canonical
 * surfaces); this page is the browsable hub + the film↔section bridge.
 */
      ...
      <div className="blg">
        <section className="blg-hero">
          <div className="blg-wrap blg-hero__grid">
            <div>
              <p className="blg-kick"><span className="dot" /> The Metatake question desk</p>
              ...
              <p className="intro" style={{ marginTop: 8 }}>Also from Metatake: <Link href="/blog">Between Film and the World — the daily →</Link></p>
            </div>
            ...
            <p style={{ margin: "10px 0 14px", fontSize: 14 }}>
              {DESK_KEYS.filter((k) => (deskCounts[k] ?? 0) > 0).map((k, i) => (
                <span key={k}>
                  ...
                  <Link href={`/curious/${k}`}>{DESKS[k].label} ({(deskCounts[k] ?? 0).toLocaleString()})</Link>
                </span>
              ))}
            </p>
            {essays.map((e, i) => {
