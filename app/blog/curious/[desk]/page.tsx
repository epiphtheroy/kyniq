...
import { DESKS, deskByKey, mdToPlain } from "@/lib/desks";
 ...
 * reading pages live at /film/[slug]/[desk].
 */
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${desk.label} — ${desk.deskName} · Curious · the Metatake blog`;
  const description = `${desk.blurb} ${n.toLocaleString()} films and counting — every essay fact-checked and verified.`;
  return {
    title,
    description,
    alternates: { canonical: `/blog/curious/${deskKey}` },
      ...
      <div className="blg">
        <section className="blg-hero">
          <div className="blg-wrap">
            <p className="blg-kick"><span className="dot" /> The metatake blog · Curious · {desk.deskName}</p>
            ...
            <p className="intro" style={{ marginTop: 8 }}><Link href="/blog/curious">← Curious (all questions &amp; desks)</Link></p>
          </div>
        </section>
