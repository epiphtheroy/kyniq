...
export async function filmLocationsMetadata(slug: string, locale: Locale): Promise<Metadata> {
  ...
  return {
    title,
    description,
    // ko canonical is self; the page is mixed-language (Korean chrome over English
    // place data) so it is not indexed until the pin names are localized (§6.5).
    alternates: { canonical: locale === DEFAULT_LOCALE ? `/film/locations/${slug}` : `/ko/film/locations/${slug}` },
    openGraph: { title, description, locale: LOCALES[locale].ogLocale },
    twitter: { card: "summary_large_image", title, description },
    robots: pageRobots(locale === DEFAULT_LOCALE), // ko noindex until pin names are localized
  };
}
...
export async function FilmLocationsPage({ slug, locale }: { slug: string; locale: Locale }) {
  const data = await load(slug);
  if (!data) notFound();
  ...
  return (
    <div className="mt">
      ...
      <div className="mt-wrap" style={{ maxWidth: 880, padding: "28px 20px 40px" }}>
        ...
        {setting.length > 0 && (
          <section style={{ margin: "28px 0" }}>
            <h2 className="df-h2">{t(locale, "The world it pretends to be")}</h2>
            <p className="df-sub">{t(locale, "Places the story claims as its world — distinct from where the cameras stood.")}</p>
        ...
        <section style={{ margin: "30px 0 0" }}>
          <h2 className="df-h2">{t(locale, "Keep reading")}</h2>
          <p style={{ lineHeight: 1.9, margin: "6px 0 0" }}>
            <Link href={`/film/${film.slug}`}>{t(locale, "{title} — analysis, themes & symbols →", { title: film.title })}</Link>
            {directorHasLocations && film.director_slug ? (
              <><br /><Link href={`/director/${film.director_slug}/locations`}>{t(locale, "Where does {director} film? Every location across the filmography →", { director: film.director ?? "" })}</Link></>
            ) : null}
            {countries.filter((c) => hubCountries.has(countrySlug(c.name))).slice(0, 3).map((c) => (
              <span key={c.name}><br /><Link href={`/locations/${countrySlug(c.name)}`}>{t(locale, "Movies filmed in {country} →", { country: countryPhrase(c.name) })}</Link></span>
            ))}
            <br /><Link href="/locations">{t(locale, "The world map of cinema — every country, one map →")}</Link>
          </p>
        </section>

        <aside lang={locale === DEFAULT_LOCALE ? undefined : "en"} style={{ margin: "34px 0 0", padding: "16px 18px", borderLeft: "3px solid #c0392b", background: "rgba(22,35,63,.04)", borderRadius: "0 6px 6px 0" }}>
