...
- [x] **D7. 하드 로그인 게이트 채택 여부 (2026-08-23, 오너 제기 → 기각·방향전환).**
      제안: 콘텐츠를 조금만 보여주고 구글 로그인으로 가입을 요구할 것인가?
      **판정 = 기각.** 대신 **P0을 "가입 압박"에서 "두 번째 페이지"로 교체한다.**
      근거·실측 전문 = **§15**. 실행 도면 = **`HANDOFF-두번째페이지-P0-설계.md`**(신설).
...
## §15. D7 재판정 — "게이트를 세울 것인가" (2026-08-23)

오너 질의: *"컨텐츠를 조금만 보여주고 구글 로그인을 요구할까, 아니면 다 보여주고 로그인을 유도할까."*
**답: 둘 다 아니다. 후자를 이미 하고 있고, 그것이 실패한 이유는 게이트의 세기가 아니다.**

### 15.1 실측 — 유도는 이미 한계까지 돌아갔고 0이었다 (mt_events, 60일)

| 이벤트 | 노출 | 전환 | 비율 |
|---|---|---|---|
| `nudge_shown:join:*` (readplates 778 · film-main 476 · directorplates 119 · film-catalog 100 · director-main 93 · hub 82) | **1,648회**, 고유 ~1,100명 | `nudge_click` **2회** | **0.12%** |
| `onetap:shown` | 759회 (632명) | 기록된 전환 0 | 0% |
| `gate_shown:*` (JIT, §5의 "최고 레버") | **10회** | `gate_method:*` **4회** | **40%** |

**JIT 게이트 설계는 옳았다 — 전환율 40%다. 60일에 10번밖에 안 뜰 뿐이다.**
행동하려는 사람은 가입한다. 행동하려는 사람에 도달하지 못하는 것이 문제다.

### 15.2 그 앞단 — 90%가 한 페이지만 본다

- 실방문자 **75.2명/일**(8/3 기준선 30명의 2.5배) · PV/방문자 **1.31**
- 세션 **90.0%가 정확히 1 PV**(2,511/2,790, 21일). 2 PV 이상 10.0%, 3 PV 이상 5.7%
- 체류 중앙값 23.4초이나 **36.7%가 60초 이상, 34%가 스크롤 60%↑** — 읽는 사람은 실재한다
- 1페이지 이탈 착지 상위: `/film/*` 787PV · `/whereto` 367PV(320명) · `/locations` 244PV · `/movies-like` 128PV

### 15.3 하드 게이트가 기각인 세 가지 이유

1. **산수.** 하루 게이트 노출 ≈ 75. 현행 CTR 0.12%를 **10배 개선해도 하루 1명**. 그 대가로 매일
   심독자 25명(60초+)을 잃는다. 최대 이익 < 최소 손실.
2. **유일하게 살아있는 유통 채널 절단.** 21일 검색 유입: DDG 271 · Bing 210 · Yahoo 41 · Ecosia 36 ·
   **Google 24(14명)**. 구글은 하루 1명이고 **첫 회복 판정이 2026-08-31**이다
   ([[gsc-traffic-drop-2026-07-18-diagnosis]]). 그 직전 콘텐츠 봉쇄 = 자해.
3. **AI 배포 축과 양립 불가.** ChatGPT 10 · Copilot 6 · Perplexity 3 유입이 이미 있고
   MCP 서버·컨텍스트 팩이 전략의 한 축이다([[ai-distribution-surface]]). 로그인 벽은 그 축을 통째로 끊는다.

### 15.4 반증은 이미 끝나 있다
**2026-08-16 하루에 11명이 가입했고 그중 9명이 활동 0**, 대부분 그날 이후 재방문 없음.
전체 33명 중 유의미한 활동 ~9명. 뉴스레터 구독자 4명.
→ **가입 수를 늘려도 아무 일도 안 일어난다는 것이 실측으로 끝났다. 가입은 결과지 원인이 아니다.**

### 15.5 방향 전환 — P0 교체

| | 이전 P0 | **새 P0 (D7)** |
|---|---|---|
| 가정 | 가입 요청이 부족하다 | **두 번째 페이지가 없다** |
| 레버 | 넛지 빈도·공격성 | **읽기의 끝 = 다음에 볼 영화** |
| 천장 | 0.12% × 75/일 | 1PV→2PV 10% → 20% = 가입 후보군 2배 |

**핵심 발견: 만들 것이 거의 없다.** `film_next` 테이블에 **17,317행**(reason 있는 행 17,216, 실제 필름
페이지로 연결 13,620)이 이미 있고 **visible 필름 1,959편 중 1,953편(99.7%)** 이 커버된다.
문제는 그것이 (a) 필름 메인의 안 눌리는 탭 안에 있고 (b) 롱테일 표면엔 아예 없다는 것뿐이다.
그리고 `ReadPlates.tsx:272`에서 **JoinCard가 탐색 플레이트보다 위에** 앉아 "어디로 갈까"를 막고 있다.

**실행 도면 = `HANDOFF-두번째페이지-P0-설계.md`** (S1~S5 · 마이그 0 · 신규 라우트 0).

### 15.6 후속 순서 (D7 확정)
- **P0** 두 번째 페이지 — 위 문서
- **P1** 게이트 발화 늘리기 — 필름 메인 히어로의 `EntityActions`는 Follow/Like뿐(`components/EntityActions.tsx`).
  판단 행동(볼래/봤어/별점)이 웹 히어로에 없다. 목표: `gate_shown` 60일 10회 → **주 10회**
- **P2** 이메일 = 계정보다 낮은 문턱인데 구독자 4명. 심독 완료 지점(스크롤 60%↑)에 1필드 캡처
- **P3** 표면 분업: **웹 = 완전 개방(유통) / 앱 = 계정 우선(관계)**. 다운로드한 사용자는 의도가 이미 증명됐다

### 15.7 게이트를 재검토할 자격이 생기는 시점 (명시적 트리거)
미터링(월 N편 무료)은 **더 읽고 싶은 압력**이 있을 때만 작동한다. 1.31 PV/방문자에는 압력이 없다.
**PV/방문자 ≥ 2.5 AND 주간 재방문자 ≥ 150** 이 동시에 성립하면 그때 A/B 대상이 된다.
현재 1.31 / 47. 재방문자는 W31 5 → W32 17 → W33 26 → **W34 47**로 상승 중이며, **이 곡선이 현재
유일하게 건강한 신호다. 이것을 부수는 결정은 하지 않는다.**
...
- **2026-08-23 v3 — D7 재판정(§15).** 오너의 하드 로그인 게이트 제안 = **기각**. 60일 실측이 근거:
  join 넛지 1,648노출/클릭 2(0.12%) · One-Tap 759노출/전환 0 · JIT 게이트 10노출/전환 4(40%).
  병목은 게이트 세기가 아니라 **세션 90%가 1페이지**라는 것. **P0을 "두 번째 페이지"로 교체**하고
  실행 도면을 `HANDOFF-두번째페이지-P0-설계.md`로 분리. 발견: `film_next` 99.7% 커버(신규 개발 불요),
  `ReadPlates.tsx:272` JoinCard가 탐색 플레이트 위에 앉아 있음.
