...
> ## ⭐⭐⭐ v4 — GSC 재실측 + 서브페이지 가지치기 (2026-07-28, Opus 세션 — 라이브)
> **이 문서의 두 번째 반쪽.** §0~§7·v2·v3 = **enrich**(신호 채워 Tier-2를 게이트 위로 → 색인 ↑).
> v4 = 그 반대이자 보완인 **prune**(얇은 **서브페이지**를 noindex → 크롤 예산 회수 → 좋은 페이지가 색인).
> "가지치기로 성장(prune to grow)".
>
> **① GSC 프로덕션 재실측 (2026-07-24/27, GSC 직접 열람):**
> - 페이지: **색인 4,641 / 미색인 5,248**(알려진 ~9,900, 색인율 ~47%). 페널티 없음·색인 성장 중.
> - 미색인 실체: **크롤됨-미색인 2,515 · 발견됨-미색인 2,156(검증 시작됨) · noindex 483(의도) + 소수(≈94)**.
> - 동영상: **색인 129 · "보기페이지에 없음" 2,176(검증 실패지만 3,800→2,176 감소 중 = StillHero 수정 잔여치 소멸, 페널티 아님)**.
> - **주소변경 filmcurio→metatake 진행 중**(GSC 라이브 확인). 완료 시 링크자산 통합 = 색인/트래픽 플러스.
>
> **② 정밀 진단 (①사이트맵·링크 / ④동영상 — 코드 확인, 둘 다 깨끗):**
> - 사이트맵 lastmod = 실제 콘텐츠날짜만(정석) · robots = 검색봇 전면허용·중요경로 차단0 · 필름메인→서브 내부링크 풍부 · 게이트(`lib/filmGate.ts`)가 페이지타입별로 이미 정교(reception≥8·credits≥2·meaning≥5·lineage≥3·필름메인 figures≥3).
> - 동영상: 필름메인 영상마크업 **0**(`InviteVideo`는 `false &&` 죽은코드·StillHero=정지이미지, VideoObject는 /tv에만). → 2,176은 100% 재크롤 대기 잔여치. **손댈 것 없음.**
> - **결론: 미색인 벽은 버그 아님 = "권위 + 크롤예산 + 프로그래매틱 서브페이지 포화".**
>
> **③ 크롤됨-미색인 2,515 실제 URL 표본 분류 (GSC 드릴다운 `item_key=CAMYFyAC`):**
> 압도적 = **얇은 필름 서브페이지** — `[desk]`183·meaning172·reception154·필름메인(얇은)114+·credits60 · **movies-like 188** · takescore 118(이미 noindex 잔여) · trope/director/concept 등 엔티티 ~270 · /tv 96.
>
> **④ 실행 = 두 누수만 정확히 막음 (커밋 `59df45a`, 라이브·tsc20):**
> - **`/film/*/[desk]`(+/ko)**: `pageRobots(true)`(**바 없던 유일한 누수**) → **에세이 `minutes≥3`(~550단어)**. 실한 비평 유지·얇은 stub noindex. (튜닝: 꼬리 크면 4로.)
> - **`/movies-like/*`**: recs 바 **3→6** + 사이트맵(`moviesLikeEntries` n_affinities≥6) 정합.
> - `takescore/film` = 이미 `pageRobots(false)`. meaning(n≥5)·reception(≥8) = **이미 바 있음 → 안 건드림**(효과 측정 후 조정, **플립플롭 금지**).
>
> **⑤ noindex 원칙(오너 Q&A 확정):** noindex ≠ 삭제(페이지 살아있음·사용자·내부링크 그대로). **영원 아님** — 게이트가 서버에서 매 렌더 재평가 → 채워서 바 넘으면 **자동 재색인**. 재신청은 대부분 자동(사이트맵+재크롤); 수동요청은 최고가치 소수만. **"채운 뒤 + 도메인신뢰 오른 뒤" 나중에 몰아서**가 유리(지금 얇은 채 강제요청은 무효).
>
> **⑥ 모니터링(2~4주, 재크롤 주기):** 크롤됨-미색인 → noindex제외로 이동(2,515↓) · 예산 풀려 발견됨-미색인 2,156 크롤·색인 · 색인총계 초반 미세하락 후 건강회복. **이 코호트 색인요청 금지 · 8/5 트래픽 재점검 · 주소변경 완료 확인.** 다음 조정 = 이 배치 효과 본 뒤 meaning/reception 순. 정본 진단·수치는 [[gsc-crawled-not-indexed-fix]].
>
> ---
>
