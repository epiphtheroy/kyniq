...
## §0.6 실사 2026-08-05 — 앱 등록·비공개 테스트 제출 완료

> 📒 그날의 전체 원장(애플 포함·미완 목록·함정): `docs/WORKLOG-2026-08-05-양대스토어-제출.md` — **재개 시 여기부터.**

전화번호 인증(마지막 계정 게이트) 통과 → 앱 레코드 생성 → 비공개 테스트 구성 → Google 검토 제출까지 하루에 완료.

```
앱 레코드   : Metatake · net.metatake.app · 앱 ID 4972847492483878062 · en-US · 무료
트랙        : 비공개 테스트 Alpha (트랙 ID 4700037821019995388)
번들        : metatake-1.0.0-vc2.aab (1.0.0 / vc2) 업로드·인식 완료 (API 24+, 타겟 SDK 36, ABI 4종)
테스터      : 이메일 목록 "Metatake Closed Testers" 14명 · 의견 이메일 wonwoo@metatake.net
국가        : 한국·미국·영국·캐나다·오스트레일리아 (5개)
앱 콘텐츠   : 전 항목 완료 — 방침 /privacy · 앱 액세스(이메일 OTP 안내) · 광고 없음 · IARC 전체이용가(ESRB E·PEGI 3)
              · 타겟층 18+ · 데이터 보안(공유 없음·전송 암호화·계정삭제 URL=/privacy) · 정부/금융/건강 아니오
              · 카테고리 엔터테인먼트 · 연락처 wonwoo@metatake.net/metatake.net · 광고 ID 사용 안 함
등록정보    : listing-en.md 텍스트 3종 + 아이콘(링 완전 채움 신판, assets/images/icon.png→512) + 그래픽 1024×500
              + 폰 스크린샷 4장(1080×1920 라이트 테마)
제출        : 2026-08-05 변경사항 14개 검토 제출 → 사전검사 후 Google 검토(보통 며칠~최대 7일)
```

**승인 후 순서:** ① 트랙 테스터 탭의 **참여 링크** 복사 → 테스터 14명에게 배포(전원 안드로이드 기기) → 12명+ 옵트인 14일 연속 유지 ② 설정→앱 무결성에서 SHA-256 지문 복사 → §7 assetlinks.json 교체 ③ 14일 후 프로덕션 액세스 신청(§2-5).

**남은 손질(비차단):** 다크모드 안드로이드 스크린샷 재캡처→교체(오너 요청·현재 라이트) · 태블릿 스크린샷(선택) · 프로덕션 전 `/account-delete` 전용 페이지(현 삭제 URL=/privacy) · R8 매핑파일 업로드(경고만).

**함정 기록:** 스토어 이미지 드래그는 브라우저 파일탭으로 새기 쉽다 — **애셋 추가→패널→Add from Drive**(Drive "메타테이크 이미지" 폴더) 경로가 확실. 아이폰 캡처(1284×2778≈9:19.5)는 Play 규격(16:9/9:16) 위반이라 업로드 자체가 안 된다(§shots-android/README 참조).
