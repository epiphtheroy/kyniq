...
## ② 번역 적재 (인바이테이션·트로프·아크타입·이론가·figures … 한국어 → content_i18n)
```
! node scripts/load-content-i18n.mjs --locale ko
```
`[content_i18n:ko] done: N rows.`가 나오면 완료. (먼저 확인만 하려면 `--dry` 붙여 실행.)
...
- **DB 매칭 구조**: `content_i18n` 중앙 테이블(다국어 재사용) + `dbLabel` 접근자 + 로더. 다음 언어(일본어·스페인어)는
  같은 구조에 언어 열만 추가.
- **번역**(content_i18n 배치, `data/i18n/content/ko/`): 인바이테이션 1,958·트로프 4,710·택소노미/아크타입 2,949·
  이론가 3,760·figures 18,381 등 — 병렬 에이전트가 자연스러운 한국어로 번역(영화명·인명 표준 표기 포함).
