# App Store Connect — 붙여넣기 시트 (build 15, v1.0)

`listing-en.md` / `listing-ko.md`에서 기계적으로 추출한 값. 이 파일만 보고 위에서
아래로 채우면 리스팅이 완성된다. 글자수는 추출 시점에 재측정했다.

경로: appstoreconnect.apple.com → Metatake → 왼쪽 **App Store** → 1.0 준비 중
...
핵심만: 대부분 None, **폭력·성적내용·욕설·성인주제·공포·약물 = Infrequent/Mild**
(영화 포스터/스틸과 인용된 비평문에서 나올 수 있으므로), **제한 없는 웹 접근 = No**
(인앱 리더는 metatake.net만 열고, 영화/감독 링크는 앱으로 가로챈다).
예상 결과 **12+**. ⚠️ 17+로 계산되면 성적내용·주제 두 항목을 다시 보고 수락할 것.

## App Privacy 답변 — `store/PRIVACY-LABELS.md` 참조

## 심사 노트 — `store/REVIEW-NOTES.md` 전문을 App Review Information에 붙여넣기
