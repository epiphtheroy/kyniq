...
| `HANDOFF-모바일앱-프리워치.md` | **모바일 앱 ("Pre-Watch Companion") 정본 v3.1 (2026-07-17, iOS+Android 시제품 완성 — 스토어 제출만 오너 TODO)** — 보기 전에 여는 결정 도구·다국가 에디션(US 출시, 국가 선택만으로 KR/ES/JP·웹 LOCALES 결합). **폰으로 보기 = `cd mobile && ./start-local.sh`**(IP 자동정렬+QR). ⚠️**Expo SDK 54 고정**(Expo Go 상한 — 올리면 폰 검토 경로 사망, 불변식 §13-13)·⚠️**앱 디자인 토큰은 웹과 분리**(§3 "Lava" — 웹 DESIGN-SYSTEM 이식 금지, 불변식 §13-14)·⚠️**읽기 표면 라우트 개명 시 앱 웹뷰 경로 3파일 동기**(§16.5 — 유일한 수동 결합점). 핵심 색인: §3 디자인시스템 · §5.5 지도 4렌더러 · §7 BFF/SSO · §8 양 플랫폼 · §13 불변식 14조 · §15 AS-BUILT/오너TODO · §16 데이터 연동 지도. 모바일 앱 작업은 여기서 시작. |
