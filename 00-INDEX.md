...
| `HANDOFF-모바일앱-프리워치.md` | **모바일 앱 ("Pre-Watch Companion") 기획 정본 v2.1 (2026-07-16, P0~P3 구현 완료·§15 AS-BUILT·스토어 제출은 오너 TODO)** — 보기 전에 여는 결정 도구·다국가 에디션(미국 출시→국가 선택만으로 KR/ES/JP 전환). 2층 구조(네이티브 결정층: TS·Invitation·Where to watch·Lineage·The Life·찜/Seen·지도 + 웹뷰 읽기층) + 접착제 4(검색·계정·Seen·푸시). BFF·SSO 핸드오프·iOS 우선(Expo 양플랫폼)·TestFlight KPI 게이트·불변식 12조. 모바일 앱 작업은 여기서 시작. |
