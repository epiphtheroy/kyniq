...
| `HANDOFF-모바일앱-프리워치.md` | **모바일 앱 ("Pre-Watch Companion") 정본 v3.0 (2026-07-17, iOS+Android 시제품 완성·스토어 제출만 오너 TODO)** — 보기 전에 여는 결정 도구·다국가 에디션(US 출시, 국가 선택만으로 KR/ES/JP 전환·웹 LOCALES 결합). **§3 디자인 시스템 v2 "Lava"**(Airbnb 2025 벤치마크: 그라데이션 CTA·블러 크롬·스프링 모션·시트 오버 포토·벤치마크 로그인 순서 — 웹 토큰 이식판은 폐기, PT Serif만 작품 제목에 생존). 2층 구조(네이티브 결정층 + 웹뷰 읽기층)·BFF 7라우트(`/api/v1/app/*`)·푸시 크론·마이그 0106·**지도 4렌더러**(§5.5, Android는 WebView MapLibre로 Google 키 회피). **§16 데이터 연동 지도** = 앱↔사이트 전 접점과 "같이 반영" 체크리스트. §15 AS-BUILT=검증 기록·오너 TODO. 모바일 앱 작업은 여기서 시작. |
