...
| `HANDOFF-모바일앱-프리워치.md` | **모바일 앱 ("Pre-Watch Companion") 정본 v2.3 (2026-07-17, 완전 동작 시제품·스토어 제출만 오너 TODO)** — 보기 전에 여는 결정 도구·다국가 에디션(US 출시, 국가 선택만으로 KR/ES/JP 전환·웹 LOCALES 레지스트리 결합). 2층 구조(네이티브 결정층: TS·Invitation·Where to watch·Lineage·The Life·찜/Seen·지도 3면 + 웹뷰 읽기층) + BFF 7라우트(`/api/v1/app/*`)·푸시 크론·마이그 0106. **§16 데이터 연동 지도** = 앱↔사이트 전 데이터 접점과 "같이 반영" 체크리스트(웹 RPC·user_movies 단일 원장·The Meter 계측·라우트 개명 시 수동 결합점 3파일). §15 AS-BUILT=검증 기록·오너 TODO. 모바일 앱 작업은 여기서 시작. |
