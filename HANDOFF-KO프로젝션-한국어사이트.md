...
- **Embedding Fantasia 모듈(df-know)의 문장 본문은 번역 대상이 아니다** — §1.1. ko 페이지에서도 영어 원문 그대로 렌더.
...
- ko 전용 컴포넌트 파일 생성 금지. `if (locale === "ko")` 분기로 레이아웃을 바꾸는 것도 금지 — 텍스트/값만 달라진다.
  (단 하나의 예외: §6.2의 robots 강화 조건은 locale 분기가 맞다 — 레이아웃이 아니라 색인 신호이므로.)
...
### 1.1 ⚠️ 예외 명시 — Embedding Fantasia 모듈은 t()로 덮이지 않는다
...
**v1 결정: ko 페이지에서도 이 모듈은 영어 원문 그대로 둔다** (섹션 라벨 "Embedding Fantasia"는 어차피 제품 어휘로 원문 유지).
한국어판은 마스터 계층 B의 "13패턴+슬롯 사전 ko 재생성"(LLM 0 유지) 프로젝트로만 가능 — §9 오너 미결로 이관.
같은 이유로 **필름 메인에 노출되는 takes(take_title·rationale)·figures(label·description)·리셉션 인용(headline·comment)·
Strong Misreadings 티저도 v1에서 영어 원문 유지**다(리셉션 인용은 원문 병기가 원칙이라 영구히 원문이 맞음). 이것이 §6.5의
혼합 언어 리스크의 원천이므로 반드시 §6.5의 코호트 선별과 함께 운용할 것.
...
- **결정론 조립 문장만 t() 대상이다**: 필름 페이지의 렌더-시 조립 리드(수상 다이제스트 C1, 개봉 원장 문장, 촬영지 리드 등 —
  ...
  커버한다. 조립 로직(부분집합·문장형 선택)은 공유. **반면 DB에서 완성문으로 오는 것(film_sentences·takes·figures·리셉션
  인용)은 t() 대상이 아니다 — §1.1의 예외 그대로 영어 렌더.** 이 구분을 헷갈리면 안 된다: "이 문자열이 코드 리터럴/템플릿인가,
  DB 행인가"가 판별 기준.
...
Tier-1 영화의 ko 페이지는 본문 상당 부분이 영어 원문(§1.1)이라, 구글이 EN 쌍둥이의 중복으로 판정하고 **캐노니컬을
EN으로 접을(folding) 위험**이 있다. 2026-07-16(56759a6)에 EN에서 "수천 편 동일 템플릿 지문"으로 색인 거절을 정리한
...
- **Phase 2 — 필름 메인 /ko**: `_shared.tsx` 추출 → ko 셸 → 결정론 리드 템플릿 `t()` 경유(§4.1 판별 기준 준수) →
  ...
  AC: 대표 4편 — ①티어1 유명작(영어 프로즈 섹션이 영어 원문으로 안전 렌더 + 게이트 판정 확인)
...
| Embedding Fantasia ko판 | v1 영어 원문 유지 (§1.1) | 마스터 계층 B "13패턴+슬롯 ko 재생성"(LLM 0 유지) 착수 — 문장층 브랜드 계약 때문에 반드시 오너 결정 |
| 롱폼(takes·figures·리셉션) ko | 영어 원문 유지 | 마스터 §6 content_i18n 루프 착수 (~$220, 오너 예산 결정) |
