...
1. **Big film banner (not a tiny strip).** A cinematic banner: backdrop image, a **large poster**,
   ...
   and clickable — the small strip in the old build is explicitly rejected.
...
4. **Stills** — a small TMDB strip near the top (Kyniqbot, §3.3).
5. **Canonical answer (the hero)** — answer-first TL;DR then the synthesis, with the Kyniq
   Editorial / voice byline + "read by {N}". Always visually dominant.
...
8. **Contributions stream — "Readings from the community"** — Top / Newest; upvote-only; merged
   markers.
9. **Related on YouTube** — the §3.3 Kyniqbot module (relevance-gated; see the matching rules in
   the media mission).
