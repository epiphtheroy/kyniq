...
> ## ✅ 상태: 웨이브 1(한국어) SHIPPED — 프로덕션 라이브 2026-07-17
>
> **이 문서가 다국어(ko·ja·fr·es) 작업의 정본이다.** 일본어·스페인어를 시작하려면 → **§-2 "새 언어 추가 절차 8단계"**
> 로 바로 가라. 코어(레지스트리·`t()`·`locVal`·셸 패턴·사전·게이트·사이트맵·스위처·배너)는 전부 라이브라, 새 언어는
> **구조 변경 0**으로 §-2.2 체크리스트만 밟으면 된다. AS-BUILT 상세: `Outputs/locale-projection-as-built.md`.
>
> **웨이브 1(ko) 라이브 현황** (커밋 `7e64d7f`, main·프로덕션 READY):
> - 코어: `lib/i18n/{locales,index,values,genres,seo,cities}.ts` + `dict/ko.ts` + `cities/ko.json`(도시 511개) +
>   `components/i18n/{LocaleProvider,SetHtmlLang,LocaleSwitcher,LocaleSuggestBanner,EnglishOriginalLabel}.tsx` +
>   `scripts/{i18n-audit.mjs,load-locations-i18n.mjs}` + `worker/tmdb-i18n-backfill.py`.
> - 로컬라이즈된 표면: **필름 메인**(`/ko/film/[slug]`, 두 렌더분기 전량)·**감독**(`/ko/director/[slug]`, noindex)·
>   **필름 촬영지**(`/ko/film/locations/[slug]`, noindex)·공유 컴포넌트(AccessSummary·FilmReceptionSection).
> - DB: 마이그 **0105 적용됨**(`title_ko`·`overview_ko`·`ko_fetched_at`·`film_locations.name_ko` nullable). TMDB ko
>   백필 실행됨(제목 ~87%·줄거리 ~68%). 사이트맵 `films-ko.xml` + hreflang(조건부) + 네비 스위처 라이브.
> - 라이브 검증: EN `/film/*`=200 무손상 · KO `/ko/film/*`=200 한국어 렌더 · `og:locale ko_KR`·`inLanguage ko` · EN에 `한국어` 스위처.
>
> **웨이브 1 남은 후속(선택, ko 확장)** — 다음 언어와 무관하게 언제든:
> - 아틀라스 국가/도시 허브 ko 셸(`/ko/locations/*`, `/ko/atlas/*`) — cities/ko.json은 준비됨, 페이지 셸만.
> - 홈 ko(`/ko`, 라이트)·카탈로그 ko.
> - `film_locations` 26k 지명 번역 배치(로더 `scripts/load-locations-i18n.mjs` 준비됨; CSV `data/i18n/film-locations-ko.csv`).
> - 롱폼(takes·figures·리셉션 본문) ko = 마스터 문서 content_i18n 루프(별도 예산).
>
> **⚠️ 다음 언어(ja·es) 착수 전 필독**: ① `lib/i18n/locales.ts`에서 `live:false`→`true` 전환이 마지막 단계
> (그 전엔 어느 표면에도 안 나옴). ② 지명·제목은 **번역이 아니라 데이터** — TMDB `ja-JP`/`es-ES` 백필 + 도시명
> 직접 번역. ③ 감독/촬영지 페이지는 혼합언어라 noindex 유지(§6.5). ④ 오너 게이트 = §2.2.1 코어 어휘 매트릭스의
> 해당 언어 열 승인. ⑤ 마이그·백필·배포는 **오너가 실행**(에이전트 샌드박스는 프로덕션 쓰기 차단 — 회원님이 `!`로).
>
> ---

> ## ✅ 상태 2: 전면 ko 로컬라이즈 (DB 구조 워딩 + 참조 영화 제목) SHIPPED — 라이브 2026-07-17
>
> 웨이브 1(위)이 라우트·크롬·필름 메인을 프로젝션했다면, **이 단계는 /ko를 "온전히 한국어로 보이게"** 하는 오너 지시
> (2026-07-17)의 이행이다. 두 축 — **DB 구조 워딩 번역**(content_i18n)과 **참조 영화 제목 오버레이**. 전부 프로덕션 라이브·검증됨.
>
> **A. DB 구조 워딩 번역 → `content_i18n` (마이그 0107)**
> - 중앙 매칭 테이블 `content_i18n(entity_type, entity_key, field, lang, text, …)` PK 4열. 접근자 `lib/i18n/dbLabel.ts`
>   (`loadLabels(locale,type,keys)→Map` · `dbLabel(map,…,en)` · `enumLabel`). 로더 `scripts/load-content-i18n.mjs`
>   (배치 JSON `data/i18n/content/ko/` → PostgREST 업서트; **PK-dedup 접기** 필수 — 없으면 SQLSTATE 21000).
> - **적재 완료 = ko 21,561행**(DB 집계): 트로프 9,420 · 택소노미/아크타입 4,818 · 이론가 4,777(이름+블러브) ·
>   인바이테이션 1,898 · lineage_list 624(영화제/수상/정전 라벨+설명) · frame 24. anon 읽기 200·한국어 라벨 확인.
> - 렌더 배선: `app/film/[slug]/_shared.tsx`가 figure·trope·taxonomy·invitation 라벨을 dbLabel로 투영(ko만).
> - **figures(≈18k label)는 오너 결정으로 미실시** — dbLabel 영어 폴백이라 무영향. 재개 시 `data/i18n/src/figures.json`
>   → 전용 워크플로우(라벨 일괄, 재개형). 배치는 **에이전트가 실시간 번역**(월 지출한도 걸리면 리셋 후 재개; 배치 API 금지).
>
> **B. 참조 영화 제목 오버레이 → `lib/i18n/filmTitles.ts` (신규)**
> - 문제: `title_ko`(마이그 0105·1,943편=주요작 87%)가 **필름 상세 H1에만** 쓰이고, 다른 영화를 참조하는 표면은
>   RPC의 영어 `title`을 그대로 렌더 → /ko 본문·목록의 참조 제목이 영어.
> - 해결: 배치 오버레이 `loadFilmTitles(locale, slugs)→Map` + `filmTitle(map,locale,slug,en)` (anon·1h캐시·소스로케일/
>   에러時 빈맵=SEO안전). **프로젝션 3표면 전량 배선·라이브검증**: ① 필름페이지(추천·Watch-next/킨드레드·counterpoints·
>   connected, 두 렌더분기; ITMFL→중경삼림·타락천사) ② 감독페이지(필모·픽스·히어로; 왕가위→아비정전·동사서독)
>   ③ 에세이 ko페이지(`[desk]/ko` film.title_ko; ko에세이 1,613편/1,394영화).
> - **범위 확정**: `/ko`로 프로젝션되는 페이지는 **딱 3종(필름·감독·촬영지)**. 그 외 40+파일(검색·트로프·장르·개념 등)은
>   **영어 전용 표면**이라 거기 영화 제목이 영어인 건 정상(페이지 전체가 영어). 촬영지는 place-names 보류와 일관되게 영어 geo-산문 유지.
> - ~~**TMDB 재백필 무효 확인**: title_ko 없는 5,035편 **전부 tmdb_id 보유**(0 없음) → TMDB에 한국어 제목 없음, 재백필 0추가.~~
>   **← 대체됨(2026-07-17)**: 이 판정은 primary-field 방식 기준. `d0bb8ab`이 추가한 **`/translations`+원제 해석 경로는
>   실제로 제목을 찾아냄**(화양연화·기생충 등) — `--refill` 부분 실행으로 NULL 5,035→**3,629**. 잔여 재개 런북·심야
>   에티켓·스크립트 수정판(no-op 스킵)은 `HANDOFF-DB성능-인시던트.md` §4. 그래도 남는 NULL은 LLM 번역(오너 스킵) 또는 영어 폴백.
>
> **배포**: `7436e60`(로더 PK-dedup 수정) → `6d95c52`(필름 참조제목) → `bd6afa1`(감독 참조제목) → `851eb41`(에세이 ko 제목).
> ⚠️ **"프로덕션 쓰기=classifier 차단"은 부분 반증**: git push는 에이전트가 성공, 그러나 **마이그·서비스롤 적재는 여전히
> classifier 차단**(오너 `!` 실행). `apply-sql.py`는 상대경로를 `worker/` 기준 해석하니 **절대경로** 필수. 캐시: 적재 후
> /ko DB라벨 반영에 unstable_cache 3600s(최대 1h).
>
> **아침 런북/재개 절차**: `Outputs/ko-full-localization-runbook.md`. 진행상태 메모리: `memory/ko-full-localization-overnight.md`.
>
> ---

> ## ⚠️ 상태 3: DB 성능 인시던트 여파 (2026-07-17) — 백필·배포 에티켓 변경
>
> 웨이브1 출시일에 **DB 포화 인시던트** 발생(백필 films 43% churn + 배포 9회 캐시초기화 + /ko 크롤 웨이브 중첩
> → Micro 인스턴스 CPU 포화·OOM 재시작). **정본: `HANDOFF-DB성능-인시던트.md`** (원인·조치·백로그 전부 그쪽).
> 이 문서에 미치는 영향만 요약:
> - **컴퓨트 Micro→Small(2GB) 업그레이드됨**(+$5/월) — /ko URL 공간 확장을 버티기 위한 조치.
> - **ko 백필 중단됨**(kill, NULL 3,629 잔여) — 재개는 인시던트 문서 §4 런북으로(심야·수정판 스크립트·no-op 스킵).
> - `worker/tmdb-i18n-backfill.py` **동작 변경**(워킹트리, 미커밋): 못 찾은 필드 NULL 덮어쓰기 제거·no-op PATCH 스킵.
> - **새 규칙**: 대량 --persist 백필은 심야+배포일 회피 / i18n류 연쇄 커밋은 배포 배칭(배포=ISR 전체 초기화).
>
> ---
...
> 체크리스트 8단계(1~7=구조 변경 0, 8=DB 워딩 번역 데이터)**.
>
...
### -2.2 새 언어 추가 절차 (체크리스트 — 이 8단계 밖의 작업이 필요하면 설계 위반을 의심할 것; 1~7=구조/데이터 투영, 8=DB 워딩 번역)
...
| 8 | **DB 구조 워딩 번역** (상단 "상태 2" 참조 — /ko가 온전히 보이게 한 층; **1~7단계만 하면 트로프·이론가·인바이테이션 등이 영어로 남으니 누락 금지**) | `data/i18n/content/<loc>/`에 배치 번역(에이전트 실시간·배치 API 금지) → `content_i18n`에 `lang='<loc>'`로 적재(`scripts/load-content-i18n.mjs --locale <loc>`, PK-dedup 내장). 대상=트로프·택소노미/아크타입·이론가·인바이테이션·lineage_list·frame(ko 21,561행 참조). **figures(≈18k)는 오너 결정**. dbLabel 렌더 배선·`filmTitles.ts` 오버레이는 **로케일-제네릭이라 데이터만 채우면 자동 동작**(코드 변경 0 — title_<loc>는 4·5단계 산물). |
...
| TMDB ko 제목 공백 보완 | 영어 폴백. **TMDB 소스 소진 확인(2026-07-17)**: title_ko 없는 5,035편 전부 tmdb_id 보유 → TMDB에 ko 제목 없음, **재백필 0추가(재실행 금지)** | KOBIS(영화진흥위원회 오픈API) 한국 공식 개봉명 조인 (마스터 계층 C — 별도 소형 워커) → 그래도 남으면 LLM 번역(오너 예산). 타 언어권 소스는 착수 시 검토 |
...
- **v5 (2026-07-17)** — **전면 ko 로컬라이즈 SHIPPED**(상단 "상태 2" 블록이 정본): ① DB 구조 워딩 → `content_i18n`
  (마이그 0107·`dbLabel`·로더 PK-dedup) 적재 ko **21,561행**(트로프·택소노미·이론가·인바이테이션·lineage·frame),
  figures는 오너 스킵 ② 참조 영화 제목 오버레이 `lib/i18n/filmTitles.ts` — 필름·감독·에세이 ko 3표면 배선+라이브검증,
  프로젝션 대상은 3종뿐(그 외는 영어전용) ③ TMDB 재백필 무효 확인(5,035편 tmdb 있으나 ko 없음→LLM 필요) ④ 배포
  7436e60/6d95c52/bd6afa1/851eb41, git push는 에이전트 직접·마이그/적재는 오너 `!`. 지도 place-names는 오너 보류 유지.
