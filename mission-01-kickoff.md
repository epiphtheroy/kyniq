...
1. Write **one SQL migration** creating the v1 tables from §4. Every table gets
   ...
   - **questions** — `film_id` FK, `author_id` FK profiles, `title`, `body`, `slug text
     unique`, `view_count int default 0`, `question_type text`, and the **lifecycle/provenance
...
2. Add foreign keys and helpful indexes: `questions(film_id)`, `questions(question_type)`,
...
5. **Seed:** insert the §11 **badge** rows, a `system`-role **"Kyniq Editorial"** profile
...
- The "Kyniq Editorial" `system` profile and the 5 seed films exist; the films are SELECTable
  with the anon key.
