...
## 오너 검토 (주 1회, ~15분)
`state/review-queue.md`를 열고 각 줄 `[ ]`에 **F**(Feature — 소개감)/**L**(List — 관측 로그행)/**R**(Reject) 표기. Watchlist 항목은 아직 빈 페이지인 유망 이름 — 다음 주에 다시 열어본다. Feature 3개 이상 모이면 다이제스트 1편 발행(P1, 정본 §4·§7).

⚠️ **후보 도메인은 일상 브라우저로 직접 열지 말 것** — 후보 풀의 ~20%가 해적판·잠재 악성이다(실측). 확인은 격리 프로필로.
