...
and the brand assets (`kyniq-*.svg/.png`, `favicon.*`, `apple-touch-icon.png`,
...
3. **Design system:**
   ...
   - Load **Fraunces**, **Newsreader**, **Hanken Grotesk** (via `next/font` or the Google Fonts
   ...
   - Build a **root layout** with the global header + footer exactly per `ref-chrome.html`:
     header logo = `kyniq-wordmark.svg` (links home); footer = About / Contact / Community
     guidelines / Terms / Privacy links, company line (Kyniq · address placeholder ·
     contact.kyniq@gmail.com), the **TMDB attribution** sentence, and "© 2026 Kyniq."
4. **Brand assets:** copy every `kyniq-*` and `favicon.*` / `apple-touch-icon.png` /
   `icon-192.png` / `icon-512.png` into `/public`. Create `site.webmanifest` (name "Kyniq",
   icons 192 + 512, `theme_color #1A2740`, `background_color #FAF7F0`). Wire all icon `<link>`s
   + the manifest in the root `<head>`, plus default Open Graph / Twitter tags using
   `kyniq-logo-paper.png`.
5. **Placeholder home (`/`)** that renders **server-side** and shows the Kyniq masthead +
...
6. **Config & hygiene:**
   ...
   - Baseline `app/robots.ts` (or `/public/robots.txt`) per §8.3 — allow the AI retrieval bots
     and reference `https://kyniq.io/sitemap.xml`.
   - `app/sitemap.ts` per §8.5 (may return just `/` for now).
   - Set `NEXT_PUBLIC_SITE_URL=https://kyniq.io`.
...
- The page renders in the editorial design (navy `#1A2740` on warm paper; Fraunces / Newsreader
  / Hanken) and the header + footer match `ref-chrome.html`.
