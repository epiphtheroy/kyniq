...
> ⚠️ **SUPERSEDED by `mission-pipeline-worker-kickoff.md`** (autonomous, multi-model,
> category-free). Kept for reference only — do not build this simpler version.
...
2. **Generate (draft).** AI writes a question + a canonical answer (and optionally 1–2
   ...
   authored by the **"FilmCurio Editorial"** `system` profile (or a disclosed editorial voice).
...
6. **No-sockpuppet / no fake engagement (hard rule, §3.2).** Never create human-looking
   ...
   real-user-only. Render the disclosure ("Drafted with AI, reviewed by the FilmCurio editorial
   team").
