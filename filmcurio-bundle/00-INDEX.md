# 00-INDEX — FilmCurio handoff (read this first)

- `REBRAND-HANDOFF.md` — Kyniq→FilmCurio rebrand + design migration guide for another AI: old→new mapping, canonical assets, every codebase touchpoint (incl. the **admin logo**), and verification steps.

Everything needed to build **FilmCurio** (filmcurio.com) in Google Antigravity. Drop the files into the
...
│  ├─ redesign-home-and-question.md ← home + question-page redesign brief (+ ref-*-v2 mockups, bugs)
...
│  ├─ ref-settings.html
│  ├─ ref-about.html
│  └─ (copy the filmcurio-*/favicon* assets here too so the refs render)
└─ public/                      ← SHIPPED static assets (served at site root "/")
   ├─ favicon.svg  favicon.ico  favicon-16.png  favicon-32.png
   ├─ apple-touch-icon.png  icon-192.png  icon-512.png
   ├─ filmcurio-wordmark.svg  filmcurio-wordmark-dark.svg     ← header/footer logo (preferred)
   ├─ filmcurio-wordmark.png  filmcurio-wordmark-dark.png     ← raster fallback
   └─ filmcurio-logo.png  filmcurio-logo-dark.png  filmcurio-logo-paper.png  ← full lockups (OG/About)
```
...
URL to the site-root path `/filmcurio-wordmark.svg` (and `/filmcurio-wordmark-dark.svg`), since the
...
- `mission-media-embed-kickoff.md` — **Curiobot**: media model + **auto-embedding** (TMDB images
...
- `globals.css` — design tokens + component classes (navy `--ink #16233F`, ivory `--bg
  #FBF8F1`, marigold `--accent #E0922A` + teal `--accent-2 #159A92`; Reddit Sans).
...
- `ref-question-media.html` — question page **with the Curiobot "Related on YouTube" bottom
  module + TMDB stills** (the media design mockup).
- `ref-home-v2.html`, `ref-question-v2.html` — **redesign mockups** (see
  `redesign-home-and-question.md`): image-rich home (hero/collections/directors/feed/rankings) +
  question page with a big film banner, boxed related-questions, and a sticky rail.

**Brand & icons** (`/public`)
- Wordmark: `filmcurio-wordmark.svg` / `-dark.svg` (vector, preferred) + `.png` fallbacks.
- Lockups: `filmcurio-logo.png` / `-dark.png` / `-paper.png` (full lockup w/ "Film Interpretation
...
- AI content carries a **transparent editorial byline** — "FilmCurio Editorial" (or a small, fixed,
