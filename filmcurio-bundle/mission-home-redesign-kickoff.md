...
> **Authoritative spec for this work: `redesign-home-and-question.md`** (detailed findings, the
> module-by-module redesign for the home AND the question page, the bugs to fix, and data notes).
> Visual mockups: **`ref-home-v2.html`** and **`ref-question-v2.html`**. Build to those.
...
starting point, but make it markedly richer. Keep the **navy / ivory** palette with **marigold + teal** accents, **Reddit Sans**, hairlines + whitespace — *density, not clutter*; never a stocky AI
look.
...
4. **Design discipline.** Strong typographic hierarchy, generous but purposeful whitespace,
   hairline dividers, accent (marigold; teal for interactive cues) used sparingly. Dark mode (incl. the logo swap). Responsive
...
- The home is visibly **dynamic** — real TMDB imagery anchors it and the activity modules show
  live, real data (no placeholders) — yet still reads as the FilmCurio editorial brand, not a generic
  template.
