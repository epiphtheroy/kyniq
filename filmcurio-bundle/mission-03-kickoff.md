...
**Context.** Read `AGENTS.md` and `SPEC.md` §6.7 (auth screens), §6.5 (ask flow), §3.1
(frictionless participation), and match `ref-signup.html` +
...
3. **Ask flow `/ask`** — keep it **light and conversational** (§3.2): step 1 pick film (uses
   the M2 TMDB search; supports `/ask?film=…`), step 2 question title + optional context —
   **no category/type select**, step 3 optional first reading. Requires login; **preserve the
   draft through the auth gate** and return to the exact action.
...
- A logged-in user's question appears under its film with `status='published'` (no category
  field anywhere).
