...
### ➕ 상세(show) 페이지 6종 — 미리 만들어 둔 디자인

상단 메뉴(인덱스)에서 한 항목을 눌러 들어가는 **개별 상세 페이지**들. 인덱스(02~05)와 짝을 이루며, 각각 **실제 현재
페이지 구조 그대로 + Supabase 실데이터**로 만들었다. 폴더: `/Users/jerryje/Documents/MetaTake/detail-pages/` · 상세 = **§6C**.

| # | 페이지 | 파일 | 라우트 | 예시(실데이터) |
|---|---|---|---|---|
| D1 | **Film** | `01-film.html` | `/film/[slug]` | Parasite |
| D2 | **Figure** | `02-figure.html` | `/film/[slug]/figure/[figureSlug]` | Parasite → the scholar's rock |
| D3 | **Director** | `03-director.html` | `/director/[slug]` | Quentin Tarantino |
| D4 | **Trope** | `04-trope.html` | `/trope/[slug]` | The Mid-Story Genre Turn |
| D5 | **Meta take** | `05-metatake.html` | `/take/[slug]` | The Cyborg Body (주인공) |
| D6 | **Ask** | `06-ask.html` | `/ask` | grounded Q&A |
...
## 6C. ➕ 상세(show) 페이지 6종 — `detail-pages/` · 미리 만들어 둔 디자인

인덱스(02~05)가 "목록"이라면, 이 6종은 **그 안의 한 항목**을 펼친 페이지다. 전부 **실제 `app/.../page.tsx` 구조를
그대로 따르고**(섹션 순서·로직 보존), 내용은 **Supabase 실데이터**로 채웠다. 인덱스/홈/블로그와 톤·메커니즘 통일.

### 공통 상세 페이지 규약 (6종 통일 — 그대로 가져갈 것)
- v6 토큰 + 마스트헤드(+Blog), 해당 nav 항목 빨강 active. **컬러 이미지**. 빨강 하나(트로프 영역만 틸).
- **스탯 스트립** — 숫자 클릭 → 해당 섹션 점프(`scroll-margin-top`) + **스크롤 진입 시 카운트업**.
- **살아있는 맵(검은 별자리)** — 엔티티별 이웃(드래그·줌·hover). 실제 반영 시 `EntityGraphLoader`/`NodeGraph` 재사용.
- **본문 전문 노출**(figure 설명·take rationale) — 클램프/펼치기 없이 다 보이게.
- **레지스터 배지 = §7 실제 매핑**, **via [figure]·via [meta take]=빨강 / 트로프=틸**.
- 같은 행에서 **figure 글씨 = 영화 제목 글씨 크기**(트로프·메타테이크·All-takes 공통 규칙).

### 페이지별 (무엇 / 진짜·가짜)
- **D1 Film** `/film/[slug]` (`app/film/[slug]/page.tsx`) — 백드롭+포스터 히어로 · "reads X through N figures…" 인트로
  · **Figures(kind 그룹·와이드 행·설명 전문·reaches[메타테이크])** · 맵 · **Meta takes | Tropes 2단(각 via 피겨)** ·
  **Films most connected(via 메타테이크)** · Film info(접이식) · seq/prov. (예: Parasite — 11 figures/19 meta/3 tropes/6 connected, 전부 실데이터.)
- **D2 Figure** `/film/[slug]/figure/[figureSlug]` — Kind/Type(트로프)/Takes · 영화 컨텍스트 · **설명 전문** · Search
  images/clips↗(외부) · **Takes(레지스터 색 좌선 카드 + rationale 전문 + → 메타테이크 허브, emerging/Community 배지)** ·
  맵 · **Connected figures(공유 트로프별 형제 figure)** · Contribute · seq/prov.
- **D3 Director** `/director/[slug]` — 인물 사진·출생 · fingerprint 인트로 · **Signature meta takes(presence dots
  N/총편수 + 영화목록)** · **Signature tropes(틸)** · **Filmography(백드롭 카드+편당 meta take 수)**. "signature"는
  **필모 ≥2편 반복**(추측 금지).
- **D4 Trope** `/trope/[slug]` (**틸**) — laconic + thesis · 맵 · **Figures of [트로프] — N across M films(실시간 검색
  필터 + 멤버 행 = 영화 — figure[동일 크기] + 설명 전문)**.
- **D5 Meta take(주인공·빨강)** `/take/[slug]` — laconic + *after [이론가]* + thesis · 맵(defining 안쪽 / unexpected kin
  바깥) · **Representative takes = Defining cases(`rel_rank`) + Unexpected kin(`surp_rank`, "표면은 멀어도 속은 한 가족"),
  각 레지스터 배지 + rationale 전문, 영화—figure 동일 크기 인라인** · **All takes(검색·🎲 Random·장르/레지스터 토글,
  폴더 아이콘 + "N takes", 펼치면 그 폴더의 전 행 표시)** · seq/prov.
- **D6 Ask** `/ask` (`app/ask/page.tsx`) — 근거 기반 Q&A. 질문 → **코퍼스 close readings에서만 합성**한 답변 +
  **인라인 [N] 인용(→ 출처 하이라이트)** · Threads to pull · **Sources(추적 가능: figure · 영화 → 메타테이크 + 레지스터)**
  · "▦ retrieved, not generated" 스탬프. §6 메시징 원칙의 결정판.

- **가짜→교체:** prev/next(SeqNav) · Save/Contribute 액션 · **Ask 답변 합성(데모 = 로딩→고정 샘플)** · 일부 `#` 링크
  → 실연동 시 실제 라우트·`/api/ask`(코퍼스 RAG)·SeqNav로. 본문/관계/랭킹/카운트는 이미 실데이터(시드).
...
7. **➕ 상세 페이지 반영**(§6C) — `detail-pages/` 6종을 실제 `app/.../page.tsx`에 입힌다. 공통 규약(스탯 점프+카운트업·
   살아있는 맵·본문 전문·via 색·figure=영화 글씨 크기) 유지. Ask는 `/api/ask`(코퍼스 RAG)로, 추측 답변 금지.
8. **유지 검증**(§12).
...
- [ ] ➕ **상세 페이지 6종**(Film·Figure·Director·Trope·Meta take·Ask)이 실제 구조대로 반영됐다. 공통 규약(스탯 점프+카운트업·살아있는 맵·**본문 전문 노출**·via 색·**figure 글씨=영화 글씨 크기**·폴더 펼치면 전 행) 유지.
- [ ] Ask는 `/api/ask`(코퍼스 RAG)로 동작하고 **모든 [N] 인용이 실제 take로 추적**된다(추측 답변 0). Meta take의 Defining/Unexpected는 `rel_rank`/`surp_rank`.
...
- **"film·figure·director·trope·meta take·Ask 상세 페이지도 — 현재 페이지 정보 보며 하나씩"** → 상세 페이지 6종(§6C, `detail-pages/`), 전부 실데이터.
- 상세 페이지 미세 피드백(반영): figure 설명 **전문**을 와이드 행으로(클램프 제거) · 메타/트로프 **2단** · 상단 숫자 **클릭 점프 + 카운트업** · 트로프/메타테이크/All-takes에서 **figure 글씨 = 영화 글씨 크기** · 폴더 펼치면 **전 행** 표시 + 폴더 아이콘·"N takes".

---

*이 문서: `/Users/jerryje/Documents/MetaTake/HANDOFF-MASTER-redesign-시안-총정리.md` · 6개 본체 시안(홈 v6 "The Pair") + ➕ Blog 3종(`blog-design/`) + ➕ 상세 페이지 6종(`detail-pages/`) · 다른 AI 반영용.*
